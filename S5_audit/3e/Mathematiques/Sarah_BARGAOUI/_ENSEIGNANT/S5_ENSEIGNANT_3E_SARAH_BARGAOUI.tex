...
\usepackage{nexusS5}
\setnexusheader{Dossier enseignant — confidentiel}{S5 — Sarah BARGAOUI — usage enseignant}
\begin{document}
\nexuscover{SÉANCE 5 --- DOSSIER ENSEIGNANT}{Profil, déroulé, corrigés et barème}{Sarah BARGAOUI}{Entrée en Troisième}{Mathématiques --- Installer l'algèbre et les équations}
...
\begin{methodbox}
\textbf{Diagnostic initial.} Diagnostic initial (18 items). Socle numérique solide ; calcul littéral à rectifier ; équations, proportionnalité, géométrie, trigonométrie et statistiques à installer.
\end{methodbox}
\subsectiontitle{Points d'appui documentés}
\begin{itemize}\item Nombres relatifs\item Fractions\item Puissances\end{itemize}
\subsectiontitle{Difficultés documentées}
\begin{itemize}\item Réduction avec signes\item Équations\item Proportionnalité\item Pythagore longueur manquante\item Trigonométrie\item Moyenne\end{itemize}
...
M3E\_FRAC\_01 & Somme et différence de fractions & S1 & acquis & OK \\
M3E\_FRAC\_02 & Produit et quotient de fractions, simplification & S1 & acquis & OK \\
...
M3E\_EQ\_01 & $\bullet$ Équation du premier degré, inconnue dans les deux membres & S2 & en voie & P1 \\
M3E\_LIT\_01 & Distributivité : développer k(ax+b) & S2 & fragile & P1 \\
M3E\_LIT\_02 & $\bullet$ Réduction d'une expression avec constantes signées & S2 & fragile & P1 \\
M3E\_GEO\_01 & Théorème de Pythagore : calcul d'une longueur & S3 & en voie & P2 \\
M3E\_PROP\_01 & Proportionnalité : quatrième proportionnelle et vitesse & S2, S5 & en voie & P2 \\
M3E\_TRIG\_01 & Choix et emploi d'un rapport trigonométrique (cosinus, sinus) & S4 & en voie & P2 \\
...
\subsectiontitle{Second axe (phase 3, 20 min) --- Résoudre une équation du premier degré et vérifier la solution}
...
\par\vspace{1mm}\textbf{Compétence visée :} \code{M3E_EQ_01}.
...
35--55 min & Résoudre une équation du premier degré et vérifier la soluti & Pas d'exemple guidé : l'élève démarre seul. Intervention uniquement sur demande. & Deux réussites autonomes ; le contrôle est écrit sans qu'on le demande. \\
...
\end{itemize}
\subsectiontitle{Phase 3 --- Résoudre une équation du premier degré et vérifier la solution}
\begin{enumerate}
\item $3x=18$ donc $\mathbf{x=6}$.
\item $2x=14$ donc $\mathbf{x=7}$.
\item $2x+6=14$ donc $2x=8$ et $\mathbf{x=4}$.
\item $2x=-10$ donc $\mathbf{x=-5}$.
\end{enumerate}
\minititle{Erreurs probables}
\begin{itemize}
\item \textsc{CONCEPT} --- signe non changé lors du passage d'un membre à l'autre
\item \textsc{METHODE} --- division effectuée avant le regroupement
\item \textsc{CONTROLE} --- solution non vérifiée
\end{itemize}
...
\begin{graybox}\small\textbf{Réponse attendue :} $5x - 7$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $8x-3x=5x$\item $6-13=-7$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_LIT\_02 & automatisme & $5x-7$ exact \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} prérequis de la mise en équation d'un problème
\par\vspace{2mm}
\itemhead{A6 --- \code{M3E_EQ_01} --- item individualisé}{1}{1.5}
\begin{graybox}\small\textbf{Réponse attendue :} $x = 5$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $5x = 25$\item $x = 5$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_EQ\_01 & procedure & $x=5$ avec au moins une étape écrite \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} outil de résolution des problèmes de Troisième
\par\vspace{2mm}
...
\begin{graybox}\small\textbf{Réponse attendue :} $210$ pages ; $AB = 7$ cm.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $84 \div 6 = 14$ pages par minute ; $14 \times 15 = 210$\item $AB^2 = 625 - 576 = 49$ donc $AB = 7$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_PROP\_01 & procedure & nombre de pages exact, relation écrite avant les nombres \\
c2 & 1 & M3E\_GEO\_01 & procedure & $AB = 7$ cm, relation de Pythagore écrite \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} proportionnalité et Pythagore, présents dans tous les sujets de brevet
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{C2 --- \code{M3E_LIT_01} --- item individualisé}{2}{4}
\begin{graybox}\small\textbf{Réponse attendue :} $14x - 4$ ; contrôle : $5\times(-1) + 4 + 11 = 10$ et $14 - 4 = 10$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $10x - 15 + 4x + 11$\item $14x - 4$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 0.5 & M3E\_LIT\_01 & procedure & développement $5(2x-3)=10x-15$ exact \\
c2 & 0.5 & M3E\_LIT\_02 & procedure & réduction en $14x-4$ exacte \\
c3 & 1 & M3E\_LIT\_02 & controle & contrôle pour $x=1$ mené sur les deux écritures \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} chaîne développer-réduire-contrôler, attendue dès septembre
\par\vspace{2mm}
...
A6 & M3E\_EQ\_01 & 3E\_TI\_Q09 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
C2 & M3E\_LIT\_01 & 3E\_TI\_Q07 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
\end{tabularx}
...
\begin{itemize}\item Indicateurs du dossier : trois réductions justes, trois équations vérifiées, deux Pythagore sans confusion, moyenne interprétée.\item Vérifier que la relation ou la propriété est écrite avant tout nombre.\item Le socle numérique étant solide, ne pas y consacrer de temps : le levier est algébrique.\end{itemize}
