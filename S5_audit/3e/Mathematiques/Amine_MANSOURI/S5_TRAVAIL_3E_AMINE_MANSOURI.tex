...
\usepackage{nexusS5}
\setnexusheader{Entrée en Troisième — Stage de pré-rentrée}{Séance 5/5 — Amine MANSOURI}
\begin{document}
\nexuscover{SÉANCE 5 --- RÉINVESTIR, CONSOLIDER, PRÉPARER LA RENTRÉE}{Livret de travail}{Amine MANSOURI}{Justification explicite et calibration de la confiance}{Entrée en Troisième --- Mathématiques --- 1\,h\,15 de travail, puis 45 minutes d'évaluation dans un document séparé}
\begin{goalbox}
\textbf{Objectif personnel de cette séance.} Rendre explicite la procédure utilisée — la réussite existe, la justification manque — et installer le choix d'un rapport trigonométrique.
\par\medskip
...
\certline
\needspace{62mm}\par\vspace{3mm}
\phasehead{Phase 3 --- Consolidation, second axe}{35--55 min}{Identifier les côtés puis choisir le rapport trigonométrique}
\begin{methodbox}
...
\begin{enumerate}
\needspace{18mm}
\item $ABC$ est rectangle en $A$. Par rapport à l'angle $\widehat{C}$, nommer $AB$, $AC$ et $BC$.
\worklines{3}
\needspace{18mm}
\item $ABC$ est rectangle en $A$ avec $AC = 7$ cm et $BC = 25$ cm. Calculer $\cos\widehat{C}$ puis $\widehat{C}$ au degré près.
\worklines{3}
\needspace{18mm}
\item $ABC$ est rectangle en $A$ avec $AB = 9$ cm et $BC = 15$ cm. Quel rapport permet de calculer $\widehat{C}$ ? Le calculer, puis donner $\widehat{C}$ au degré près.
\worklines{4}
\needspace{18mm}
\item $ABC$ est rectangle en $A$, $BC = 20$ cm et $\widehat{B} = 35^\circ$. Calculer $AB$ au dixième près.
\worklines{4}
\end{enumerate}
\begin{successbox}\textbf{Critère de réussite de cette phase :} quatre rapports correctement identifiés puis employés, chaque réponse étant précédée du nom des côtés utilisés.\end{successbox}
\certline
\needspace{62mm}\par\vspace{3mm}
