...
\usepackage{nexusS5}
\setnexusheader{Dossier enseignant — confidentiel}{S5 — Amine MANSOURI — usage enseignant}
\begin{document}
\nexuscover{SÉANCE 5 --- DOSSIER ENSEIGNANT}{Profil, déroulé, corrigés et barème}{Amine MANSOURI}{Entrée en Troisième}{Mathématiques --- Justification explicite et calibration de la confiance}
...
\begin{methodbox}
\textbf{Diagnostic initial.} Diagnostic initial (18 items). Nombreuses réussites accompagnées d'une confiance très faible ; calcul littéral à installer ; fractions et trigonométrie à situer.
\end{methodbox}
\subsectiontitle{Points d'appui documentés}
\begin{itemize}\item Relatifs\item Puissances\item Équations\item Proportionnalité\item Pythagore\item Statistiques — sous réserve d'explicitation\end{itemize}
\subsectiontitle{Difficultés documentées}
\begin{itemize}\item Justifier les procédures\item Réduire avec signes\item Multiplier des fractions\item Installer la trigonométrie\item Calibrer la confiance\end{itemize}
...
M3E\_LIT\_02 & $\bullet$ Réduction d'une expression avec constantes signées & S2 & en voie & P1 \\
M3E\_TRIG\_01 & $\bullet$ Choix et emploi d'un rapport trigonométrique (cosinus, sinus) & S4 & non évalué & P1 \\
...
M3E\_LIT\_01 & Distributivité : développer k(ax+b) & S2 & en voie & P2 \\
M3E\_STAT\_01 & Moyenne simple et moyenne pondérée & S5 & en voie & P3 \\
\end{tabularx}
...
\begin{warningbox}\textbf{Effet de récence.} Les compétences suivantes sont retravaillées pendant cette séance, puis évaluées moins d'une heure plus tard : \code{M3E_LIT_02}, \code{M3E_TRIG_01}, \code{M3E_STAT_01}. Une réussite y mesure une \emph{performance immédiate}, pas une consolidation durable. Le plan de rentrée prévoit pour elles un contrôle différé en semaine 1 ou 2 ; aucun bilan ne doit écrire « acquis durablement » sur cette seule preuve.\end{warningbox}
\sectiontitle{3. Pourquoi ces activités, pour cet élève}
\begin{goalbox}\textbf{Objectif de la séance :} Rendre explicite la procédure utilisée — la réussite existe, la justification manque — et installer le choix d'un rapport trigonométrique.\end{goalbox}
\subsectiontitle{Axe prioritaire (phase 2, 25 min) --- Réduire une expression avec des constantes signées}
...
\subsectiontitle{Second axe (phase 3, 20 min) --- Identifier les côtés puis choisir le rapport trigonométrique}
Le dossier signale une trigonométrie à installer ou une confusion cosinus/sinus ; l'erreur est un défaut d'identification des côtés, non un défaut de calcul.
\par\vspace{1mm}\textbf{Compétence visée :} \code{M3E_TRIG_01}.
...
35--55 min & Identifier les côtés puis choisir le rapport trigonométrique & Pas d'exemple guidé : l'élève démarre seul. Intervention uniquement sur demande. & Deux réussites autonomes ; le contrôle est écrit sans qu'on le demande. \\
...
\end{enumerate}
\subsectiontitle{Phase 2 --- Réduire une expression avec des constantes signées}
\begin{enumerate}
\item $\mathbf{11x-11}$.
\item $\mathbf{3x+9}$.
\item $8x-12-3x+8=\mathbf{5x-4}$.
\item $-2x+12+5x-9=\mathbf{3x+3}$.
\item $-9-6-2-5=-22$ et $-11-11=-22$.
\item Les constantes ont été additionnées sans leur signe. Résultat exact : $\mathbf{6x-4}$. Contrôle : pour $x=0$, l'expression vaut $-7+3=-4$.
\end{enumerate}
...
\end{itemize}
\subsectiontitle{Phase 3 --- Identifier les côtés puis choisir le rapport trigonométrique}
\begin{enumerate}
\item $AC$ : côté adjacent à $\widehat{C}$ ; $AB$ : côté opposé à $\widehat{C}$ ; $BC$ : hypoténuse.
\item $\cos\widehat{C}=\frac{7}{25}=0{,}28$ donc $\widehat{C}\approx \mathbf{74^\circ}$.
\item $AB$ est opposé à $\widehat{C}$ : c'est le sinus. $\sin\widehat{C}=\frac{9}{15}=0{,}6$ donc $\widehat{C}\approx \mathbf{37^\circ}$.
\item $\cos 35^\circ = \frac{AB}{20}$ donc $AB = 20\cos 35^\circ \approx \mathbf{16{,}4}$ cm.
\end{enumerate}
\minititle{Erreurs probables}
\begin{itemize}
\item \textsc{CONCEPT} --- cosinus et sinus échangés
\item \textsc{LECTURE} --- côtés nommés par rapport au mauvais angle
\item \textsc{CONTROLE} --- valeur supérieure à 1 acceptée pour un cosinus
\end{itemize}
...
\par\vspace{2mm}
\itemhead{A5 --- \code{M3E_LIT_02} --- item individualisé}{1}{1}
\begin{graybox}\small\textbf{Réponse attendue :} $-3x - 3$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $6x-9x=-3x$\item $5-8=-3$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_LIT\_02 & automatisme & $-3x-3$ exact \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & coefficient négatif refusé : $3x$ au lieu de $-3x$ \\
CONCEPT & $5-8$ traité comme $+3$ \\
CALCUL & $6x-9x$ mal évalué alors que la règle est correctement énoncée \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} prérequis de la factorisation et des identités remarquables
\par\vspace{2mm}
\itemhead{A6 --- \code{M3E_TRIG_01} --- item individualisé}{1}{1.5}
\begin{graybox}\small\textbf{Réponse attendue :} $\cos\widehat{B}=\dfrac{AB}{BC}$ : c'est le cosinus.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $AB$ est adjacent à $\widehat{B}$, $BC$ est l'hypoténuse\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_TRIG\_01 & automatisme & rapport exact et nom correct \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & sinus donné à la place du cosinus \\
LECTURE & côtés nommés par rapport à l'angle $\widehat{C}$ \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} trigonométrie mobilisée dès les premiers chapitres de Troisième
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{B4 --- \code{M3E_GEO_01} --- item individualisé}{2}{4}
\begin{graybox}\small\textbf{Réponse attendue :} $AC = 8$ cm ; l'hypoténuse $BC$ est le plus grand côté du triangle rectangle.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $BC^2 = AB^2 + AC^2$\item $289 = 225 + AC^2$\item $AC^2 = 64$ donc $AC = 8$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_GEO\_01 & justification & $AC = 8$ cm, relation écrite et théorème cité \\
c2 & 1 & M3E\_GEO\_01 & controle & justification du contrôle de vraisemblance \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & $15^2+17^2$ additionnés \\
JUSTIFICATION & théorème non cité ou justification absente à la question 2 \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} la justification écrite est l'attendu central du brevet et de la Troisième
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{C2 --- \code{M3E_FRAC_02} --- item individualisé}{2}{4}
\begin{graybox}\small\textbf{Réponse attendue :} $\dfrac{1}{6}$ ; le produit se calcule terme à terme, seul le rapport global est en jeu, alors qu'une somme exige des parts de même taille.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $\frac{12}{72}=\frac16$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_FRAC\_02 & automatisme & produit exact et simplifié \\
c2 & 1 & M3E\_FRAC\_02 & justification & explication correcte de la différence de procédure \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
METHODE & réduction au même dénominateur appliquée au produit \\
JUSTIFICATION & règle récitée sans explication \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} calcul fractionnaire exact, exigé sans calculatrice en Troisième
\par\vspace{2mm}
...
A5 & M3E\_LIT\_02 & 3E\_TI\_Q08 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
A6 & M3E\_TRIG\_01 & 3E\_TI\_Q15 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
B4 & M3E\_GEO\_01 & 3E\_TI\_Q14 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
C2 & M3E\_FRAC\_02 & 3E\_TI\_Q04 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
\end{tabularx}
...
\begin{itemize}\item Le dossier documente une confiance très faible malgré de nombreuses réussites : noter systématiquement la certitude annoncée et la confronter au résultat réel.\item Exiger une explication orale courte après chaque réussite ; c'est l'indicateur retenu au dossier (quatre réponses justifiées oralement).\item Ne pas ajouter de difficulté : le levier est la verbalisation, non le niveau des exercices.\end{itemize}
