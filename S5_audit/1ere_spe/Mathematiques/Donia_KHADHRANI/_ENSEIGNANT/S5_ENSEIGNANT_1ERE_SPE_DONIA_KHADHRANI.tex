...
\usepackage{nexusS5}
\setnexusheader{Dossier enseignant — confidentiel}{S5 — Donia KHADHRANI — usage enseignant}
\begin{document}
\nexuscover{SÉANCE 5 --- DOSSIER ENSEIGNANT}{Profil, déroulé, corrigés et barème}{Donia KHADHRANI}{Entrée en Première générale — Spécialité mathématiques}{Mathématiques --- Langage fonctionnel et inéquations}
...
\begin{methodbox}
\textbf{Diagnostic initial.} Diagnostic initial (18 items). Vecteurs solides ; calcul littéral et numérique à consolider ; inéquations, fonctions, fonctions de référence et pourcentages à installer ; droites et probabilités à rectifier.
\end{methodbox}
\subsectiontitle{Points d'appui documentés}
\begin{itemize}\item Vecteurs\item Procédures disponibles en calcul littéral et numérique\end{itemize}
\subsectiontitle{Difficultés documentées}
\begin{itemize}\item Image/antécédent\item Fonctions de référence\item Inéquations\item Coefficient directeur\item Pourcentages\item Union de probabilités\end{itemize}
...
M1RE\_DROIT\_01 & Coefficient directeur d'une droite & S3 & fragile & P1 \\
M1RE\_FONC\_02 & $\bullet$ Langage des fonctions : image, antécédent, appartenance à la courbe & S2 & en voie & P1 \\
M1RE\_INEQ\_01 & $\bullet$ Inéquation du premier degré et écriture en intervalle & S1 & en voie & P1 \\
M1RE\_ALG\_01 & Développement et identités remarquables & S1 & en voie & P2 \\
M1RE\_ALG\_02 & Factorisation, en particulier la différence de deux carrés & S1 & en voie & P2 \\
M1RE\_DROIT\_02 & Lecture d'une équation réduite de droite & S3 & fragile & P2 \\
M1RE\_FONC\_01 & Image d'un réel, substitution d'une valeur négative & S2 & en voie & P2 \\
...
M1RE\_PROBA\_01 & Union, intersection et formule P(A$\cup$B) & S4 & fragile & P2 \\
M1RE\_PROBA\_02 & Événement contraire & S4 & fragile & P2 \\
...
\begin{warningbox}\textbf{Effet de récence.} Les compétences suivantes sont retravaillées pendant cette séance, puis évaluées moins d'une heure plus tard : \code{M1RE_FONC_02}, \code{M1RE_INEQ_01}. Une réussite y mesure une \emph{performance immédiate}, pas une consolidation durable. Le plan de rentrée prévoit pour elles un contrôle différé en semaine 1 ou 2 ; aucun bilan ne doit écrire « acquis durablement » sur cette seule preuve.\end{warningbox}
\sectiontitle{3. Pourquoi ces activités, pour cet élève}
\begin{goalbox}\textbf{Objectif de la séance :} Écrire ce qui est cherché puis la relation utilisée — conseil explicite du dossier — en commençant par le vocabulaire fonctionnel puis par les inéquations.\end{goalbox}
\subsectiontitle{Axe prioritaire (phase 2, 25 min) --- Le langage des fonctions : image, antécédent, appartenance à la courbe}
...
\subsectiontitle{Second axe (phase 3, 20 min) --- Inéquation du premier degré et écriture en intervalle}
Les inéquations figurent parmi les domaines à installer d'après le dossier ; elles conditionnent les tableaux de signes du second degré.
\par\vspace{1mm}\textbf{Compétence visée :} \code{M1RE_INEQ_01}.
...
10--35 min & Le langage des fonctions : image, antécédent, appartenance à & Rappel bref, exemple guidé au tableau, puis exercices en autonomie. Aide donnée seulement après une tentative écrite. & Trois réussites consécutives sans aide sur la compétence visée. \\
35--55 min & Inéquation du premier degré et écriture en intervalle & Pas d'exemple guidé : l'élève démarre seul. Intervention uniquement sur demande. & Deux réussites autonomes ; le contrôle est écrit sans qu'on le demande. \\
...
75--120 min & Évaluation finale & Individuel, sans document. Partie A environ 10 min, B environ 20 min, C environ 15 min. & Somme des durées cibles : 36 min, marge de 8 min. \\
\end{tabularx}
...
\end{enumerate}
\subsectiontitle{Phase 2 --- Le langage des fonctions : image, antécédent, appartenance à la courbe}
\begin{enumerate}
...
\item $h(5)=25-25=0$ : oui, le point appartient à la courbe.
\item Image et antécédent sont inversés. Formulations exactes : $7$ est l'image de $4$ ; $4$ est un antécédent de $7$.
\end{enumerate}
...
\end{itemize}
\subsectiontitle{Phase 3 --- Inéquation du premier degré et écriture en intervalle}
\begin{enumerate}
\item $4x\le 16$ donc $x\le 4$ : $S=\left]-\infty\,;4\right]$.
\item $-2x>6$ donc $x<-3$ : $S=\left]-\infty\,;-3\right[$.
\item $-2x \ge -8$ donc $x\le 4$ : $S=\left]-\infty\,;4\right]$.
\item $3x-4\le 20$ donne $x\le 8$ : $S=\left]-\infty\,;8\right]$.
\end{enumerate}
\minititle{Erreurs probables}
\begin{itemize}
\item \textsc{CONCEPT} --- sens de l'inégalité conservé après division par un négatif
\item \textsc{NOTATION} --- crochet fermé sur une inégalité stricte ou sur l'infini
\item \textsc{CONTROLE} --- aucune valeur testée dans l'inéquation initiale
\end{itemize}
...
\begin{keybox}\textbf{Total : 20 points sur 20.} Noyau commun 14 pts (70.0\,\%), items individualisés 6 pts (30.0\,\%). Somme des durées cibles : 36 minutes.\end{keybox}
...
\par\vspace{2mm}
\itemhead{A5 --- \code{M1RE_FONC_02} --- item individualisé}{1}{1.25}
\begin{graybox}\small\textbf{Réponse attendue :} $x = 4$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $4x-3=13$\item $4x=16$ donc $x=4$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M1RE\_FONC\_02 & procedure & $x=4$ avec l'équation écrite \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & image calculée à la place de l'antécédent \\
METHODE & antécédent trouvé par essais sans équation \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} recherche d'antécédents, permanente en Première
\par\vspace{2mm}
\itemhead{A6 --- \code{M1RE_INEQ_01} --- item individualisé}{1}{1.25}
\begin{graybox}\small\textbf{Réponse attendue :} $x \le 4$, soit $S = \left]-\infty\,;4\right]$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $-5x \ge -20$\item division par $-5$ : le sens change\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M1RE\_INEQ\_01 & procedure & solution et intervalle exacts \\
...
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & sens de l'inégalité conservé \\
NOTATION & crochet ouvert en $4$ \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} prérequis des tableaux de signes du second degré
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{B4 --- \code{M1RE_DROIT_01} --- item individualisé}{2}{5.25}
\begin{graybox}\small\textbf{Réponse attendue :} $m = -2$ ; $y = -2x + 3$ ; oui, les deux droites ont le même coefficient directeur.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $m = \frac{-1-7}{2-(-2)} = \frac{-8}{4} = -2$\item $7 = -2\times(-2)+p$ donne $p = 3$\item même coefficient directeur $\Rightarrow$ droites parallèles\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M1RE\_DROIT\_01 & procedure & coefficient directeur exact \\
c2 & 0.6 & M1RE\_DROIT\_01 & procedure & équation réduite exacte \\
c3 & 0.4 & M1RE\_DROIT\_02 & justification & parallélisme justifié par l'égalité des coefficients directeurs \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & différences inversées dans le quotient \\
CALCUL & ordonnée à l'origine non déterminée \\
JUSTIFICATION & parallélisme affirmé sans critère \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} droites et vecteurs directeurs, base de la géométrie de Première
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{C2 --- \code{M1RE_PROBA_01} --- item individualisé}{2}{3.75}
\begin{graybox}\small\textbf{Réponse attendue :} $P(A\cup B) = 0{,}6$ ; $P(\overline{A}) = 0{,}55$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $0{,}45+0{,}3-0{,}15=0{,}6$\item $1-0{,}45=0{,}55$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M1RE\_PROBA\_01 & procedure & $P(A\cup B)$ exacte, formule visible \\
c2 & 1 & M1RE\_PROBA\_02 & procedure & probabilité de l'événement contraire exacte \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & intersection non retirée \\
CONCEPT & contraire confondu avec l'union \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} prérequis des probabilités conditionnelles de Première
\par\vspace{2mm}
...
A5 & M1RE\_FONC\_02 & 1S\_TI\_Q06 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
A6 & M1RE\_INEQ\_01 & 1S\_TI\_Q03 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
B4 & M1RE\_DROIT\_01 & 1S\_TI\_Q11 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
C2 & M1RE\_PROBA\_01 & 1S\_TI\_Q15 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
\end{tabularx}
...
\begin{itemize}\item Indicateurs du dossier : quatre formulations fonctionnelles correctes, trois inéquations autonomes sur quatre, trois pentes correctes, union et complémentaire distingués.\item Vérifier que le sens de l'inégalité est explicitement discuté à chaque division par un nombre négatif.\item Les vecteurs étant solides, s'en servir comme appui pour le coefficient directeur.\end{itemize}
