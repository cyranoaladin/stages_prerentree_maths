...
\usepackage{nexusS5}
\setnexusheader{Dossier enseignant — confidentiel}{S5 — Sinda CHIKHAOUI — usage enseignant}
\begin{document}
\nexuscover{SÉANCE 5 --- DOSSIER ENSEIGNANT}{Profil, déroulé, corrigés et barème}{Sinda CHIKHAOUI}{Entrée en Quatrième}{Mathématiques --- Transfert et plan de rentrée}
...
\begin{methodbox}
\textbf{Diagnostic initial.} Diagnostic initial (18 items). Trois domaines solides ; priorité aux relatifs, fractions, calcul littéral et aire du triangle.
\end{methodbox}
\subsectiontitle{Points d'appui documentés}
\begin{itemize}\item Proportionnalité\item Géométrie\item Statistiques\end{itemize}
\subsectiontitle{Difficultés documentées}
\begin{itemize}\item Soustraction d'un négatif\item Fractions équivalentes\item Distributivité\item Réduction avec constantes négatives\item Aire du triangle\end{itemize}
...
M4E\_GEO\_01 & Somme des angles d'un triangle, triangle rectangle & S4 & acquis & OK \\
M4E\_GEO\_02 & Symétrie centrale et caractérisation du parallélogramme & S4 & acquis & OK \\
...
M4E\_AIRE\_01 & Aire et périmètre du rectangle et du triangle, unités & S2 & fragile & P1 \\
M4E\_FRAC\_02 & Fractions équivalentes et réduction au même dénominateur & S1 & fragile & P1 \\
M4E\_LIT\_01 & Distributivité simple : développer k(a+b) & S3 & fragile & P1 \\
M4E\_LIT\_02 & $\bullet$ Réduction d'une expression avec constantes signées & S3 & fragile & P1 \\
M4E\_REL\_01 & $\bullet$ Somme et différence de relatifs, dont la soustraction d'un négatif & S1 & fragile & P1 \\
M4E\_REL\_02 & Ordre des relatifs et déplacements sur une droite graduée & S1 & fragile & P1 \\
...
\begin{warningbox}\textbf{Effet de récence.} Les compétences suivantes sont retravaillées pendant cette séance, puis évaluées moins d'une heure plus tard : \code{M4E_STAT_01}, \code{M4E_LIT_02}, \code{M4E_REL_01}. Une réussite y mesure une \emph{performance immédiate}, pas une consolidation durable. Le plan de rentrée prévoit pour elles un contrôle différé en semaine 1 ou 2 ; aucun bilan ne doit écrire « acquis durablement » sur cette seule preuve.\end{warningbox}
\sectiontitle{3. Pourquoi ces activités, pour cet élève}
\begin{goalbox}\textbf{Objectif de la séance :} Stabiliser la soustraction d'un nombre négatif et la réduction d'une expression comportant des constantes négatives, deux points nommés dans le plan de remédiation.\end{goalbox}
\subsectiontitle{Axe prioritaire (phase 2, 25 min) --- Somme, différence et soustraction d'un nombre négatif}
...
\par\vspace{1mm}\textbf{Compétence visée :} \code{M4E_REL_01}.
\subsectiontitle{Second axe (phase 3, 20 min) --- Réduire une expression en respectant le signe des constantes}
...
\par\vspace{1mm}\textbf{Compétence visée :} \code{M4E_LIT_02}.
...
\end{enumerate}
\subsectiontitle{Phase 2 --- Somme, différence et soustraction d'un nombre négatif}
\begin{enumerate}
\item $8+3=\mathbf{11}$.
\item $-6-5+9=\mathbf{-2}$.
\item $12+4-7=\mathbf{9}$.
\item $-11+15=\mathbf{4}$.
\item $-7-(-5)=-7+5=-2$ : le nombre manquant est $\mathbf{-5}$.
\item Il a traité $-(-6)$ comme $-6$. Résultat exact : $\mathbf{15}$. Contrôle : $15$ est à droite de $9$, ce qui est cohérent avec une soustraction de nombre négatif.
\end{enumerate}
\minititle{Erreurs probables}
\begin{itemize}
\item \textsc{CONCEPT} --- soustraire un négatif traité comme une soustraction ordinaire
\item \textsc{CALCUL} --- erreur isolée sur une somme de deux négatifs
\end{itemize}
\subsectiontitle{Phase 3 --- Réduire une expression en respectant le signe des constantes}
\begin{enumerate}
\item $\mathbf{3x-8}$.
\item $\mathbf{3x+7}$.
\item $3x+12+2x-5=\mathbf{5x+7}$.
\item $6x-12-x+9=\mathbf{5x-3}$.
\end{enumerate}
\minititle{Erreurs probables}
\begin{itemize}
\item \textsc{CONCEPT} --- signe de la constante ignoré lors du regroupement
\item \textsc{CONCEPT} --- terme en $x$ additionné à une constante
\item \textsc{CONTROLE} --- aucune substitution de contrôle
\end{itemize}
...
\par\vspace{2mm}
\itemhead{A5 --- \code{M4E_REL_01} --- item individualisé}{1}{1}
\begin{graybox}\small\textbf{Réponse attendue :} $9$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $7+6=13$\item $13-4=9$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M4E\_REL\_01 & automatisme & $9$ exact \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} prérequis du produit de relatifs en Quatrième
\par\vspace{2mm}
\itemhead{A6 --- \code{M4E_FRAC_02} --- item individualisé}{1}{1}
\begin{graybox}\small\textbf{Réponse attendue :} $\dfrac{1}{3}$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $\frac{3}{4}=\frac{9}{12}$\item $\frac{9}{12}-\frac{5}{12}=\frac{4}{12}=\frac13$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M4E\_FRAC\_02 & automatisme & $\frac{4}{12}$ ou $\frac13$ \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & conversion partielle de $\frac34$ \\
...
\par\vspace{2mm}
\itemhead{B4 --- \code{M4E_LIT_01} --- item individualisé}{2}{5.5}
\begin{graybox}\small\textbf{Réponse attendue :} $6x-12$ ; puis $9x-7$ ; contrôle : les deux écritures valent $-7$ pour $x=0$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $6(x-2)=6x-12$\item $6x-12+3x+5=9x-7$\item $x=0$ : $-12+5=-7$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 0.5 & M4E\_LIT\_01 & procedure & développement $6(x-2)=6x-12$ exact \\
c2 & 0.5 & M4E\_LIT\_02 & procedure & réduction en $9x-7$ exacte \\
c3 & 1 & M4E\_LIT\_02 & controle & contrôle pour $x=0$ mené sur les deux écritures \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
\par\vspace{2mm}
\itemhead{C2 --- \code{M4E_AIRE_01} --- item individualisé}{2}{4.75}
\begin{graybox}\small\textbf{Réponse attendue :} Base $=12$ cm. En dupliquant le triangle par un demi-tour autour du milieu d'un côté, on obtient un \textbf{parallélogramme} de même base et de même hauteur, d'aire $\text{base}\times\text{hauteur}$ ; le triangle en est exactement la moitié.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $b = 54\times 2 \div 9 = 12$ cm\item deux triangles superposables pavent un parallélogramme de même base et de même hauteur\item l'aire de ce parallélogramme vaut $\text{base}\times\text{hauteur}$, donc celle du triangle en est la moitié\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M4E\_AIRE\_01 & procedure & base exacte avec unité \\
c2 & 1 & M4E\_AIRE\_01 & justification & explication géométrique correcte : duplication en parallélogramme de même base et même hauteur. Une explication par découpage-recollement aboutissant à un rectangle de même base et même hauteur est également acceptée si elle est correctement conduite \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} grandeurs et relations inverses, réinvesties toute l'année
\par\vspace{2mm}
...
A5 & M4E\_REL\_01 & 4E\_TI\_Q02 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
B4 & M4E\_LIT\_01 & 4E\_TI\_Q08 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
C2 & M4E\_AIRE\_01 & 4E\_TI\_Q12 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
\end{tabularx}
...
\begin{itemize}\item Vérifier que chaque règle est associée à une représentation (droite graduée, modèle d'aire) et à un moyen de contrôle, conformément au conseil du dossier.\item Repérer toute erreur reproduite avec une certitude 4 sur les notions reprises : c'est l'indicateur de réussite fixé au dossier.\item Observer si le contrôle par substitution ($x = 0$) est utilisé spontanément.\end{itemize}
