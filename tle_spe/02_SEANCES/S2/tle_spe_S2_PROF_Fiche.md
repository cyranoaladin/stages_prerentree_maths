...
1. Appliquer sans erreur exp(a) × exp(b) = exp(a+b) et exp(a)/exp(b) = exp(a−b).
...
|---:|---|---|---|
| 10 min | Ouverture | Question de contrôle sur la séance 1 : sens de variation de v_n = 4 × 0,7^n | Répond, déclare sa certitude |
...
3. **Test numérique**, à faire faire par les élèves, en x = 1 :
   - e^(2×1) / e^(1−1) = e² / e⁰ = e² ≈ 7,39 ;
   - e^(3×1−1) = e² ≈ 7,39 — l'erreur ne se voit pas en x = 1.
   Refaire en **x = 2** :
   - e⁴ / e¹ = e³ ≈ 20,09 ;
   - e^(3×2−1) = e⁵ ≈ 148,4. La contradiction apparaît.
...
**Les quatre règles**, toutes déduites de exp(a + b) = exp(a) × exp(b) :

| Règle | Écriture |
|---|---|
| Produit | exp(a) × exp(b) = exp(a + b) |
...
exp(x) = exp(x/2 + x/2) = (exp(x/2))², qui est un carré, donc positif ou nul ; et
exp(x) × exp(−x) = exp(0) = 1, donc exp(x) ne peut pas être nul.
...
| exp(a + b) écrit exp(a) + exp(b) | Test numérique avec a = b = 1 : e² ≈ 7,39 contre 2e ≈ 5,44 |
| exp(x²) confondu avec (exp(x))² | Faire calculer les deux en x = 2 : e⁴ contre e⁴ — puis en x = 3 : e⁹ contre e⁶ |
