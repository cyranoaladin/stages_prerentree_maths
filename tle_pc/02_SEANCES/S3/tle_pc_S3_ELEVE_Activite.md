# Terminale Spécialité Physique-Chimie — Séance 3 — Fiche élève
## Énergie : travail, énergies cinétique et potentielle, énergie mécanique

**Ton objectif de séance :** savoir dire *quand* une force travaille, et *quand elle ne
travaille pas alors même qu'elle s'exerce*.

### Règle de travail

- Je trace l'**angle** entre la force et le déplacement avant de calculer.
- J'écris la relation littérale **avant** de remplacer par des nombres.
- Toute énergie que j'écris porte l'unité $\si{\joule}$.
- Je note la certitude de ma réponse : $\square$1 $\square$2 $\square$3 $\square$4.
- Je note l'aide que j'ai utilisée : A, B, C, D ou E.

---

## Partie 1 — Avant tout : ta réponse spontanée

**Question 0.** Tu tires une valise à roulettes sur un sol horizontal, sur
$\SI{20}{\metre}$. Que vaut le travail du **poids** de la valise ?

Ma réponse : ..................................................  Ma certitude : $\square$1 $\square$2 $\square$3 $\square$4

Maintenant, réponds à ceci : de combien la valise est-elle montée ou descendue ?

....................................................................................................

Et si le poids avait travaillé, où serait passée cette énergie ?

....................................................................................................

Ce que je constate : .....................................................................

---

## Partie 2 — La trace écrite

> **Travail d'une force constante.**
> $$W_{AB}(\vv{F}) = \vv{F} \cdot \vv{AB} = F \times AB \times \cos\alpha$$
> où $\alpha$ est l'angle entre la force et le déplacement.
>
> | Angle $\alpha$ | Travail | Nom |
> |---|---|---|
> | $0°$ | maximal, positif | moteur |
> | entre $0°$ et $90°$ | positif | moteur |
> | $90°$ | **nul** | la force ne travaille pas |
> | entre $90°$ et $180°$ | négatif | résistant |
> | $180°$ | minimal, négatif | résistant |
>
> **Une force peut s'exercer en permanence sans jamais travailler.** C'est le cas du poids
> lors d'un déplacement horizontal : la force existe, mais elle est perpendiculaire au
> déplacement, donc elle ne transfère aucune énergie.
>
> **Les trois énergies.**
>
> | Grandeur | Expression | Dépend de |
> |---|---|---|
> | Énergie cinétique | $E_c = \frac{1}{2}mv^2$ | la **vitesse** |
> | Énergie potentielle de pesanteur | $E_{pp} = mgz$ | l'**altitude** |
> | Énergie mécanique | $E_m = E_c + E_{pp}$ | les deux |
>
> Les trois s'expriment en **joules**.
>
> **Conservation.** Sans frottement, $E_m$ reste constante : ce que l'objet perd en
> altitude, il le gagne en vitesse.

---

## Partie 3 — Entraînement

### Comment tu trouves ton parcours

Ton livret individuel porte, pour cette séance, une **posture** et un **parcours**. Le tableau
ci-dessous dit ce que tu traites. Tu ne fais pas les huit exercices : tu fais les tiens, et tu
les fais entièrement.

| Ta posture du jour | Ce que tu traites | Ce qu'on attend de toi |
|---|---|---|
| **DIAGNOSTIQUER** — tu avais laissé ce domaine sans réponse | Question 0, puis exercices 1 et 2 | Répondre même sans être sûr : déclarer une certitude de 1 est une réponse, pas un aveu |
| **CONFRONTER** — tu t'es trompé en étant sûr de toi | Question 0, puis exercices 1 à 4 | Écrire ce que tu croyais, puis ce qui l'a mis en défaut |
| **INSTALLER** — il te manque quelque chose, et tu le sais | Exercices 1 à 4 | Écrire la relation utilisée **avant** de remplacer par les valeurs |
| **CONSOLIDER** — tu réussis, sans en être sûr | Exercices 3 à 6 | Justifier par écrit, et contrôler l'unité du résultat |
| **ENTRETENIR** — c'est acquis et assumé | Exercices 6 à 8 | Contrôler l'ordre de grandeur et les chiffres significatifs |

### Exercices 1 à 4 — pistes Diagnostiquer, Confronter et Installer

**Exercice 1.** Une caisse de $\SI{15}{\kilogram}$ est tirée horizontalement sur
$\SI{8}{\metre}$ par une force horizontale de $\SI{40}{\newton}$. On prendra
$g =  \SI{9.8} {\newton\per\kilogram}$.

Travail de la force de traction : ..............................  Unité : ..........

Travail du poids : ..............................  Justification : ..........................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 2.** Un objet de $\SI{2.0}{\kilogram}$ se déplace à $\SI{6.0}{\metre\per\second}$.
Calculer son énergie cinétique. Puis recalculer si sa vitesse double.

$E_c$ à $\SI{6.0}{\metre\per\second}$ : ..............................

$E_c$ à $\SI{12.0}{\metre\per\second}$ : ..............................

Par combien l'énergie a-t-elle été multipliée ? ..................  Pourquoi ? ................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 3.** Le même objet de $\SI{2.0}{\kilogram}$ est posé, immobile, sur une étagère à
$\SI{1.5}{\metre}$ du sol. Calculer son énergie cinétique et son énergie potentielle de
pesanteur.

$E_c =$ ..............................  $E_{pp} =$ ..............................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 4.** Une bille tombe en chute libre d'une hauteur de $\SI{5.0}{\metre}$. Que vaut
le travail de son poids sur cette descente ? Ce travail est-il moteur ou résistant ?

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

### Exercices 3 à 6 — piste Consolider

**Exercice 5.** Une luge glisse sans frottement le long d'une pente. Elle part immobile
d'une altitude de $\SI{12}{\metre}$ et arrive au bas de la pente. Quelle est sa vitesse en
bas ? Utiliser la conservation de l'énergie mécanique.

Relation utilisée : ......................................................................

Calcul : ...........................................................................................

Résultat : ..............................  Unité : ..........

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 6.** Une caisse est tirée sur $\SI{10}{\metre}$ par une force de $\SI{50}{\newton}$
faisant un angle de $30°$ avec l'horizontale, dans le sens du déplacement. Calculer le
travail de cette force.

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

### Exercices 6 à 8 — piste Entretenir

**Exercice 7.** La même luge que l'exercice 5, mais cette fois avec frottement. Elle
arrive en bas à $\SI{12}{\metre\per\second}$ au lieu de la vitesse calculée sans frottement.
Sa masse est de $\SI{25}{\kilogram}$. Quelle énergie a été dissipée par frottement ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

**Exercice 8.** Un satellite décrit une orbite circulaire autour de la Terre. La force de
gravitation est dirigée vers le centre de la Terre, le déplacement est tangent à
l'orbite. Que vaut le travail de la force de gravitation sur un tour complet ? Qu'en
déduit-on sur la vitesse du satellite ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

---

## Partie 4 — Ce que la Terminale en fera

Une voiture freine et s'arrête. Son énergie cinétique valait $\SI{200}{\kilo\joule}$. Où
est passée cette énergie ?

....................................................................................................

Le bilan de Première ne suit que $E_c$ et $E_{pp}$. Quand il y a frottement, l'énergie
mécanique diminue : elle n'a pas disparu, elle est passée sous une autre forme,
l'**énergie interne**.

Le **premier principe** de la thermodynamique complète le bilan :

$$\Delta U = W + Q$$

où $Q$ est le transfert thermique. Rien n'est perdu ; tout est compté. C'est sur cette
relation que la Terminale construit le flux thermique, la résistance thermique et la loi
de refroidissement de Newton.

---

## Partie 5 — Bilan de séance

**Ce que je sais faire maintenant et que je ne savais pas faire ce matin :**

....................................................................................................

**Ma certitude sur le travail d'une force, en fin de séance :** $\square$1 $\square$2 $\square$3 $\square$4

**Le geste que je dois automatiser :** ..............................................................

**Avant la prochaine séance :** refaire l'exercice 2 sans le corrigé, et vérifier que
toutes tes réponses portent l'unité $\si{\joule}$.

---
_Source pédagogique unique : `stage_prerentree_terminale_pc.md`._
