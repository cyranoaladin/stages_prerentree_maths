# Terminale Spécialité Physique-Chimie — Cahier des cinq séances — Mayar Ourabi
## Physique-chimie — Stage de pré-rentrée 2026-2027

> **DOCUMENT CONFIDENTIEL — DONNÉES NOMINATIVES**  
> À conserver dans le dossier pédagogique de l'élève. Ne pas diffuser hors de Nexus Réussite et de la famille concernée.

**Élève :** Mayar Ourabi  
**Groupe :** Groupe 4 — Stage de physique-chimie  
**Matière :** Physique-chimie  
**Organisme :** Nexus Réussite  
**Stage :** 10 heures, 2 heures par jour, 5 jours consécutifs  
**Dates :** du 24 au 28 août 2026

---

## Ton parcours de pré-rentrée

Le thème de chaque séance est commun au groupe. L'objectif, la piste et les exercices sont les tiens : ils viennent de ton positionnement.

| Séance | Thème | Ton objectif | Ta piste |
|---:|---|---|:---:|
| 1 | Transformations chimiques : avancement, réactif limitant, oxydo-réduction | Rectifier une certitude erronée sur l'énergie. Faire apparaître, puis lever, l'idée fausse installée sur les ondes. | Confronter |
| 2 | Mécanique : vecteur vitesse, forces, vers la deuxième loi de Newton | Installer les repères indispensables en mécanique. | Installer |
| 3 | Énergie : travail, énergies cinétique et potentielle, énergie mécanique | Poser les définitions et les gestes de base en optique. | Installer |
| 4 | Ondes et optique : période, célérité, longueur d'onde, lentilles minces | Construire les premiers automatismes sur les transformations chimiques. | Installer |
| 5 | Électricité, chimie organique, mesure et incertitudes, évaluation | Stabiliser l'acquis encore fragile en chimie organique. | Consolider |

## Comment utiliser ce cahier

- Chaque séance suit le même ordre : réactivation, essentiel, méthode, entraînement, transfert, ouverture, bilan.
- Tu ne traites que les exercices de ta piste. Ils sont déjà sélectionnés ici : ce cahier ne contient pas ceux des autres.
- Écris la propriété ou la relation **avant** de calculer. C'est la seule habitude que ce stage cherche à installer partout.
- Note à chaque fois la lettre de l'aide utilisée. Ce n'est pas un aveu : c'est la mesure de ton autonomie, et on veut la voir baisser d'ici la séance 5.

---

<div class="page-break"></div>

# Séance 1 — Transformations chimiques : avancement, réactif limitant, oxydo-réduction

**Le thème du groupe aujourd'hui :** Transformations chimiques : avancement, réactif limitant, oxydo-réduction.  
**Ton point personnel pendant le temps différencié :** Énergie.  
**Ta piste :** Confronter. Écris d'abord ce que tu croyais, puis ce qui l'a mis en défaut. C'est cette trace-là qui empêche l'erreur de revenir.

## Aujourd'hui, tu vas…

Rectifier une certitude erronée sur l'énergie. Faire apparaître, puis lever, l'idée fausse installée sur les ondes.

L'entraînement collectif porte sur transformations chimiques ; ton exercice personnel, plus bas, porte sur énergie. Les deux sont à traiter : le premier avec le groupe, le second pendant le temps différencié.

> **Remarque.** Ce que ton positionnement a montré sur ce point : Énergie (CONFRONTER) · Ondes et signaux (CONFRONTER). Sur ce domaine, tu as réussi 0 % des questions du positionnement. C'est de là que part ta séance.

> **Rappel.** Un point sur lequel tu peux t'appuyer aujourd'hui : Électricité — réponses justes, données avec assurance : c'est un vrai point d'appui pour la suite.

## Pour commencer — dix minutes, de tête

Ces questions ne sont pas notées. Elles servent à réveiller ce dont la séance a besoin. Si l'une te bloque, signale-la : c'est une information utile.

1. Formule reliant quantité de matière, masse et masse molaire.  ....................
2. Formule reliant quantité de matière, concentration et volume.  ....................
3. $\SI{0.20}{\mole\per\litre}$ dans $\SI{250}{\milli\litre}$ : quelle quantité de matière ?  ....................
4. Dans un couple $\ce{Ox/Red}$, lequel gagne des électrons ?  ....................
5. Une solution de pH 3 est-elle plus ou moins acide qu'une solution de pH 6 ?  ....................
6. Que vaut l'avancement à l'état initial ?  ....................
7. Qu'est-ce qui est conservé au cours d'une transformation chimique ?  ....................

## Avant tout : ta réponse spontanée

> **Remarque.** Réponds **avant** de lire la suite, et note ta certitude honnêtement. Sur ce domaine, ton positionnement a donné une réponse fausse assurée : c'est cette réponse-là qu'il faut voir apparaître pour pouvoir la reprendre.

**Question 0.** Pour la transformation d'équation $\ce{2A + B -> C}$, on introduit
$\SI{0.4}{\mole}$ de A et $\SI{0.3}{\mole}$ de B. Quel est le réactif limitant ?

Ma réponse : ..................................................  Ma certitude : $\square$1 $\square$2 $\square$3 $\square$4

Maintenant, complète le tableau :

| État | quantité de A | quantité de B |
|---|---|---|
| Initial | ................ | ................ |
| Avancement $x$ | ................ | ................ |

Pour quelle valeur de $x$ la quantité de A s'annule-t-elle ? ..................

Pour quelle valeur de $x$ la quantité de B s'annule-t-elle ? ..................

Lequel des deux s'épuise le premier ? ..................................................

Ce que je constate : .....................................................................

---

## L'essentiel à retenir

> **Réactif limitant.** Pour chaque réactif, on calcule le quotient de sa quantité de
> matière par son coefficient stœchiométrique. Le **plus petit** quotient désigne le
> réactif limitant, et vaut l'avancement maximal $x_{\max}$.
>
> Ici : $0{,}4/2 = 0{,}2$ et $0{,}3/1 = 0{,}3$. Le réactif limitant est **A**, et
> $x_{\max} =  \SI{0.2} {\mole}$.
>
> **On ne compare jamais les quantités brutes.** Le coefficient dit à quelle vitesse le
> réactif est consommé.
>
> **Oxydo-réduction.** Un couple associe toujours un oxydant et un réducteur. Le
> réducteur **cède** des électrons, l'oxydant les **capte** :
> $$\ce{Cu^2+ + 2e^- <=> Cu}$$
> $\ce{Cu^2+}$ est l'oxydant, $\ce{Cu}$ le réducteur.
>
> **Brønsted.** Un acide **cède** un proton $\ce{H+}$ ; une base le **capte**. La définition
> par les ions $\ce{HO^-}$ est celle d'Arrhenius, et concerne les bases.
>
> **pH.** Plus le pH est **faible**, plus la solution est acide. Une unité de pH
> correspond à un facteur dix sur la concentration en $\ce{H3O+}$.

---

## La méthode, dans l'ordre

1. Je convertis toutes les données dans les unités du système international.
2. Je calcule les quantités de matière initiales de chaque réactif.
3. Je dresse le tableau d'avancement, ligne par ligne, sans sauter l'état intermédiaire.
4. Je divise chaque quantité initiale par son coefficient stœchiométrique : le plus petit quotient désigne le limitant.
5. Je contrôle que l'état final ne comporte aucune quantité négative.

## Un exemple mené jusqu'au bout

> **Exemple.** Pour $\ce{3A + 2B -> D}$, on introduit $\SI{0.6}{\mole}$ de A et
> $\SI{0.5}{\mole}$ de B.
>
> Quotients : $0{,}6/3 = 0{,}2$ et $0{,}5/2 = 0{,}25$.
>
> Le plus petit est $0{,}2$ : le réactif limitant est **A**, et
> $x_{\max} =  \SI{0.2} {\mole}$.
>
> **Contrôle.** À $x = 0{,}2$, il reste $0{,}6 - 3 \times 0{,}2 = 0$ mole de A, et
> $0{,}5 - 2 \times 0{,}2 =  \SI{0.1} {\mole}$ de B. A est bien épuisé, B est en excès.

---

## Les pièges de ce domaine

- Désigner comme limitant le réactif le moins abondant, sans diviser par le coefficient stœchiométrique.
- Oublier de convertir les millilitres en litres avant d'appliquer $n = C \times V$.
- Écrire une demi-équation sans équilibrer les charges.
- Croire qu'un pH élevé signifie une solution plus acide.

## Ton entraînement

**Exercice 1.** Pour $\ce{A + 3B -> C}$, on introduit $\SI{0.5}{\mole}$ de A et
$\SI{1.2}{\mole}$ de B. Quel est le réactif limitant ? Justifier par les quotients.

Quotients calculés : ......................................................................

Réponse : ...........................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 2.** Pour $\ce{3A + 2B -> D}$, on introduit $\SI{0.6}{\mole}$ de A et
$\SI{0.5}{\mole}$ de B. Déterminer le réactif limitant **et** l'avancement maximal.

Réactif limitant : ..................  $x_{\max} =$ ..................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 3.** Dans le couple $\ce{Fe^2+/Fe}$, identifier l'oxydant et le réducteur, puis
écrire la demi-équation électronique.

Oxydant : ..................  Réducteur : ..................

Demi-équation : ..............................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 4.** Une solution a un pH de 2. Une autre a un pH de 6. Laquelle est la plus
acide, et combien de fois sa concentration en $\ce{H3O+}$ est-elle plus grande ?

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

## Tes exercices, ceux qui viennent de ton positionnement

**1. Énergie — Calculer une énergie cinétique.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Un véhicule de $\SI{1200}{\kilogram}$ roule à $\SI{20}{\metre\per\second}$. Calculer son énergie cinétique, puis celle qu'il aurait à $\SI{40}{\metre\per\second}$.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Donne l'expression de l'énergie potentielle de pesanteur.

> **Méthode.** Ec = $\tfrac{1}{2} m v^2$, en joules. La vitesse intervient au carré : la doubler quadruple l'énergie cinétique.

**2. Énergie — Calculer le travail d'une force.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Un objet de $\SI{5}{\kilogram}$ est monté verticalement de $\SI{3}{\metre}$. Calculer le travail de son poids $(g =$ $\SI{9.8}{\newton\per\kilogram}$). Ce travail est-il moteur ou résistant ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Confond l'existence permanente d'une force avec la production d'un travail.

> **Méthode.** Le travail dépend de l'angle entre la force et le déplacement. Une force perpendiculaire au déplacement ne travaille pas, quelle que soit son intensité.

### Si tu bloques — ouvre les indices dans l'ordre

N'ouvre l'indice suivant que si le précédent n'a pas suffi, et note lequel t'a débloqué : « je ne savais pas » et « il me manquait un déclencheur » ne se traitent pas de la même façon.

- **Indice 1 — orientation.** Relis l'énoncé et demande-toi de quel domaine il relève. Écris le nom de la propriété avant toute chose.
- **Indice 2 — méthode.** Reprends la méthode de la page précédente, étape par étape, et applique la première à ton énoncé.
- **Indice 3 — première étape.** Pose l'écriture de départ, sans la résoudre. Si tu y arrives, la suite t'appartient.

Indice qui m'a débloqué : $\square$1 $\square$2 $\square$3 $\square$aucun, j'ai trouvé seul

## Transfert — à toi de choisir la méthode

Rien dans cet énoncé ne dit quelle notion employer. C'est la question.

Un comprimé effervescent contient $\SI{0.50}{\gram}$ d'hydrogénocarbonate
de sodium $\ce{NaHCO3}$ ($M = \SI{84}{\gram\per\mole}$). Plongé dans un excès d'acide, il
libère du dioxyde de carbone selon $\ce{HCO3- + H+ -> H2O + CO2}$. a) Quel est le réactif
limitant, et pourquoi n'y a-t-il pas à le chercher ici ? b) Quel volume de $\ce{CO2}$ obtient-on,
avec $V_m = \SI{24}{\litre\per\mole}$ ? c) L'ordre de grandeur est-il plausible pour un
comprimé ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

## Pour aller vers la Terminale

Ce qui suit n'est pas attendu de toi aujourd'hui. C'est là pour que tu voies à quoi sert ce que tu viens de travailler.

En Première, la transformation est supposée **totale** : l'avancement final vaut
l'avancement maximal.

En Terminale, on introduit le **taux d'avancement final** :
$$\tau = \frac{x_f}{x_{\max}}$$
Il vaut 1 pour une transformation totale, et moins de 1 lorsque le système atteint un
**état d'équilibre**.

Cinq chapitres de ton année commencent par le tableau que tu viens de poser : taux
d'avancement final, quotient de réaction, titrage, évolution spontanée, cinétique.

---

## Prendre du recul

- Comment savoir, sans tableau, qu'un réactif est en excès ?

....................................................................................................

- Quel contrôle rapide révèle une erreur de conversion millilitres-litres ?

....................................................................................................

## Avant de partir

Trois questions courtes. Elles disent au professeur ce qui est acquis, ce qui est à confirmer, et ce qui doit être repris à la séance suivante.

**1.** Écris la relation entre quantité de matière, masse et masse molaire.

....................................................................................................

**2.** $\SI{0.10}{\mole\per\litre}$ dans $\SI{50}{\milli\litre}$ : quelle quantité de matière ?

....................................................................................................

**3.** Comment désigne-t-on le réactif qui disparaît le premier ?

....................................................................................................

### D'ici la prochaine séance — quinze minutes, pas davantage

D'ici la première séance, repère une question sur l'énergie dont la réponse te paraît évidente : on la mettra à l'épreuve ensemble.

Fait : $\square$oui $\square$non    Ce que j'ai noté : ..................................................

**Ma certitude sur cette séance :** $\square$1 $\square$2 $\square$3 $\square$4  
**L'aide maximale que j'ai utilisée :** $\square$A $\square$B $\square$C $\square$D $\square$E $\square$aucune

---

<div class="page-break"></div>

# Séance 2 — Mécanique : vecteur vitesse, forces, vers la deuxième loi de Newton

**Le thème du groupe aujourd'hui :** Mécanique : vecteur vitesse, forces, vers la deuxième loi de Newton.  
**Ta piste :** Installer. Écris la propriété ou la relation que tu utilises **avant** de calculer.

## Aujourd'hui, tu vas…

Installer les repères indispensables en mécanique.

> **Remarque.** Ce que ton positionnement a montré sur ce point : Mécanique (INSTALLER). Sur ce domaine, tu as réussi 0 % des questions du positionnement. C'est de là que part ta séance.

> **Rappel.** Un point sur lequel tu peux t'appuyer aujourd'hui : Électricité — réponses justes, données avec assurance : c'est un vrai point d'appui pour la suite.

## Pour commencer — dix minutes, de tête

Ces questions ne sont pas notées. Elles servent à réveiller ce dont la séance a besoin. Si l'une te bloque, signale-la : c'est une information utile.

1. Formule du poids d'un objet de masse m.  ....................
2. Unité d'une force.  ....................
3. Poids d'un objet de $\SI{2.0}{\kilogram}$, avec $g = \SI{9.8}{\newton\per\kilogram}$.  ....................
4. Le vecteur vitesse est-il tangent ou perpendiculaire à la trajectoire ?  ....................
5. Un objet immobile : les forces se compensent-elles ?  ....................
6. Qui exerce le poids d'un objet ?  ....................
7. Un mouvement à vitesse de norme constante sur un cercle : le vecteur vitesse est-il constant ?  ....................

## L'essentiel à retenir

> **Chute libre.** Un corps est en chute libre lorsqu'il est soumis à **son seul poids**.
> Le frottement de l'air n'y figure pas : ce n'est pas un oubli, c'est la définition.
>
> Le frottement existe dans la réalité. Mais un énoncé qui parle de chute libre a décidé
> de le négliger. Si l'énoncé demande d'en tenir compte, le mouvement **n'est plus** une
> chute libre.
>
> **Vecteur vitesse.** En chaque point, le vecteur vitesse est **tangent à la
> trajectoire** et orienté dans le sens du mouvement.
>
> - La trajectoire impose la **direction**. Rien d'autre.
> - La **norme** peut varier d'un point à l'autre, même sur une courbe.
> - Une norme constante sur une trajectoire courbe, c'est le cas particulier du mouvement
>   circulaire **uniforme** — pas une règle générale.
>
> **Bilan des forces — toujours dans cet ordre.**
>
> 1. Le **système** : quel objet ?
> 2. Le **référentiel** : par rapport à quoi ?
> 3. La **liste** des forces extérieures, chacune avec son point d'application, sa
>    direction, son sens.
> 4. Seulement ensuite : une relation.
>
> **Contrôle.** Pour chaque force de ta liste, demande-toi : exercée par **quel objet** ?
> Une force sans objet qui l'exerce n'existe pas.

---

## La méthode, dans l'ordre

1. Je définis le système étudié, en une phrase, avant tout schéma.
2. Je fais l'inventaire des forces, en nommant pour chacune ce qui l'exerce.
3. Je représente chaque force par une flèche partant du centre du système.
4. Je regarde si les forces se compensent, et je conclus sur le vecteur vitesse.
5. Je contrôle l'unité de chaque résultat.

## Un exemple mené jusqu'au bout

> **Exemple.** Un pendule : une bille suspendue à un fil, en mouvement. On néglige l'action
> de l'air.
>
> - **Système :** la bille.
> - **Référentiel :** terrestre.
> - **Forces :** le poids $\vv{P}$, exercé par la Terre, vertical vers le bas ; la tension
>   $\vv{T}$, exercée par le fil, dirigée le long du fil vers le point d'attache.
>
> **Contrôle.** Chaque force a un objet qui l'exerce : la Terre, le fil. Deux forces, deux
> objets. Le bilan est complet.

---

## Les pièges de ce domaine

- Confondre masse, en kilogrammes, et poids, en newtons.
- Oublier une force de contact — la réaction du support, la tension d'un fil.
- Croire qu'un objet immobile ne subit aucune force.
- Raisonner sur la norme de la vitesse en oubliant sa direction.

## Ton entraînement

**Exercice 1.** Une balle est lâchée sans vitesse initiale. On néglige l'action de l'air.
Système, référentiel, bilan des forces.

Système : ..................................  Référentiel : ..................................

Forces : ...........................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 2.** La même balle, mais l'énoncé précise cette fois « en tenant compte de la
résistance de l'air ». Le mouvement est-il encore une chute libre ? Quel est le nouveau
bilan des forces ?

....................................................................................................

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 3.** Sur la trajectoire ci-dessous, trace le vecteur vitesse aux points
$A$, $B$ et $C$. La vitesse est plus grande en $B$ qu'en $A$ et en $C$.

```{=latex}
\begin{center}
\begin{tikzpicture}[x=13mm,y=13mm]
  \draw[Navy,very thick] plot[smooth,domain=0:5.2,samples=80] (\x,{1.4*sin(\x r)+0.2*\x});
  \foreach \x/\lab in {0.9/A, 2.6/B, 4.4/C}
     \fill[Red] (\x,{1.4*sin(\x r)+0.2*\x}) circle (2pt)
        node[below right=0.5mm,font=\small\bfseries] {$\lab$};
\end{tikzpicture}
\end{center}
```

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 4.** Un point décrit un cercle à vitesse de norme constante. Le vecteur
vitesse est-il constant ? Justifier.

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

## Tes exercices, ceux qui viennent de ton positionnement

**1. Mécanique — Caractériser le vecteur vitesse.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Un point décrit un cercle à vitesse de norme constante. Le vecteur vitesse est-il constant ? Le vecteur accélération est-il nul ? Justifier.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Généralise une propriété du mouvement circulaire uniforme à toute trajectoire courbe.

> **Méthode.** Le vecteur vitesse est tangent à la trajectoire et orienté dans le sens du mouvement. Une vitesse constante en norme n'est pas un vecteur vitesse constant.

**2. Mécanique — Reconnaître les conditions d'une chute libre.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Une bille tombe dans l'air, les frottements n'étant pas négligeables. S'agit-il d'une chute libre ? Quelles forces s'exercent alors sur elle ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Introduit une force opposée au mouvement, exclue par définition de la chute libre.

> **Méthode.** La chute libre est un **modèle** : il n'est valable que si le poids est la seule force à prendre en compte. Vérifier que les frottements sont négligeables avant de l'invoquer.

### Si tu bloques — ouvre les indices dans l'ordre

N'ouvre l'indice suivant que si le précédent n'a pas suffi, et note lequel t'a débloqué : « je ne savais pas » et « il me manquait un déclencheur » ne se traitent pas de la même façon.

- **Indice 1 — orientation.** Relis l'énoncé et demande-toi de quel domaine il relève. Écris le nom de la propriété avant toute chose.
- **Indice 2 — méthode.** Reprends la méthode de la page précédente, étape par étape, et applique la première à ton énoncé.
- **Indice 3 — première étape.** Pose l'écriture de départ, sans la résoudre. Si tu y arrives, la suite t'appartient.

Indice qui m'a débloqué : $\square$1 $\square$2 $\square$3 $\square$aucun, j'ai trouvé seul

## Transfert — à toi de choisir la méthode

Rien dans cet énoncé ne dit quelle notion employer. C'est la question.

Un skieur descend une piste rectiligne à vitesse constante. a) Que peut-on
affirmer sur la somme des forces qu'il subit ? b) Fais le bilan des forces et explique comment
elles se compensent, alors que la pente n'est pas horizontale. c) Le skieur accélère
maintenant : qu'est-ce qui a changé dans le bilan ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

## Pour aller vers la Terminale

Ce qui suit n'est pas attendu de toi aujourd'hui. C'est là pour que tu voies à quoi sert ce que tu viens de travailler.

Le **vecteur accélération** mesure la variation du vecteur vitesse. Il n'est pas nul dès
que la vitesse change — **en norme ou en direction**.

Toute la mécanique de Terminale tient dans une seule relation :

$$\sum \vv{F}_{\text{ext}} = m \, \vv{a}$$

C'est une égalité entre **vecteurs**. On la résout en la **projetant** sur des axes :
chaque projection donne une équation. C'est pour cela que le caractère vectoriel de la
vitesse, travaillé aujourd'hui, n'est pas un détail de notation.

Trois mouvements seront traités avec elle : dans un champ de pesanteur uniforme, dans un
champ électrique uniforme, dans un champ de gravitation — avec les lois de Kepler.

---

## Prendre du recul

- Comment vérifier qu'un bilan des forces est complet ?

....................................................................................................

- Vitesse constante en norme : la somme des forces est-elle nécessairement nulle ?

....................................................................................................

## Avant de partir

Trois questions courtes. Elles disent au professeur ce qui est acquis, ce qui est à confirmer, et ce qui doit être repris à la séance suivante.

**1.** Poids d'un objet de $\SI{5.0}{\kilogram}$, avec $g = \SI{9.8}{\newton\per\kilogram}$.

....................................................................................................

**2.** Le vecteur vitesse est-il tangent à la trajectoire ?

....................................................................................................

**3.** Un objet immobile subit-il des forces ?

....................................................................................................

### D'ici la prochaine séance — quinze minutes, pas davantage

Note deux ou trois réflexes que tu utilises sur les ondes : les écrire permettra de voir lequel bifurque.

Fait : $\square$oui $\square$non    Ce que j'ai noté : ..................................................

**Ma certitude sur cette séance :** $\square$1 $\square$2 $\square$3 $\square$4  
**L'aide maximale que j'ai utilisée :** $\square$A $\square$B $\square$C $\square$D $\square$E $\square$aucune

---

<div class="page-break"></div>

# Séance 3 — Énergie : travail, énergies cinétique et potentielle, énergie mécanique

**Le thème du groupe aujourd'hui :** Énergie : travail, énergies cinétique et potentielle, énergie mécanique.  
**Ton point personnel pendant le temps différencié :** Optique.  
**Ta piste :** Installer. Écris la propriété ou la relation que tu utilises **avant** de calculer.

## Aujourd'hui, tu vas…

Poser les définitions et les gestes de base en optique.

L'entraînement collectif porte sur énergie ; ton exercice personnel, plus bas, porte sur optique. Les deux sont à traiter : le premier avec le groupe, le second pendant le temps différencié.

> **Remarque.** Ce que ton positionnement a montré sur ce point : Optique (INSTALLER). Sur ce domaine, tu as réussi 0 % des questions du positionnement. C'est de là que part ta séance.

> **Rappel.** Un point sur lequel tu peux t'appuyer aujourd'hui : Électricité — réponses justes, données avec assurance : c'est un vrai point d'appui pour la suite.

## Pour commencer — dix minutes, de tête

Ces questions ne sont pas notées. Elles servent à réveiller ce dont la séance a besoin. Si l'une te bloque, signale-la : c'est une information utile.

1. Formule de l'énergie cinétique.  ....................
2. Formule de l'énergie potentielle de pesanteur.  ....................
3. Unité de l'énergie.  ....................
4. Un objet de $\SI{2.0}{\kilogram}$ à $\SI{3.0}{\metre\per\second}$ : quelle énergie cinétique ?  ....................
5. L'énergie potentielle dépend-elle de l'origine choisie ?  ....................
6. Que vaut le travail d'une force perpendiculaire au déplacement ?  ....................
7. Un objet immobile posé sur une étagère a-t-il une énergie cinétique ?  ....................

> **Rappel.** Tu as travaillé **Énergie** en séance 1. Avant de commencer, écris en une phrase la règle que tu en as retenue — sans regarder tes notes. Si elle ne vient pas, c'est le moment de le dire.

## L'essentiel à retenir

> **Travail d'une force constante.**
> $$W_{AB}(\vv{F}) = \vv{F} \cdot \vv{AB} = F \times AB \times \cos\alpha$$
> où $\alpha$ est l'angle entre la force et le déplacement.
>
> | Angle $\alpha$ | Travail | Nom |
> |---|---|---|
> | $0°$ | maximal, positif | moteur |
> | entre $0°$ et $90°$ | positif | moteur |
> | $90°$ | **nul** | la force ne travaille pas |
> | entre $90°$ et $180°$ | négatif | résistant |
> | $180°$ | minimal, négatif | résistant |
>
> **Une force peut s'exercer en permanence sans jamais travailler.** C'est le cas du poids
> lors d'un déplacement horizontal : la force existe, mais elle est perpendiculaire au
> déplacement, donc elle ne transfère aucune énergie.
>
> **Les trois énergies.**
>
> | Grandeur | Expression | Dépend de |
> |---|---|---|
> | Énergie cinétique | $E_c = \frac{1}{2}mv^2$ | la **vitesse** |
> | Énergie potentielle de pesanteur | $E_{pp} = mgz$ | l'**altitude** |
> | Énergie mécanique | $E_m = E_c + E_{pp}$ | les deux |
>
> Les trois s'expriment en **joules**.
>
> **Conservation.** Sans frottement, $E_m$ reste constante : ce que l'objet perd en
> altitude, il le gagne en vitesse.

---

## La méthode, dans l'ordre

1. Je choisis l'origine des altitudes et je l'annonce.
2. Je calcule l'énergie mécanique à l'état initial, puis à l'état final.
3. Je compare les deux : l'écart est l'énergie échangée avec l'extérieur.
4. S'il y a frottement, je nomme l'énergie dissipée au lieu de dire qu'elle a disparu.
5. Je contrôle l'ordre de grandeur du résultat.

## Un exemple mené jusqu'au bout

> **Exemple.** Une caisse est tirée sur $\SI{12}{\metre}$ par une force de $\SI{60}{\newton}$
> faisant un angle de $60°$ avec l'horizontale, dans le sens du déplacement.
>
> $$W = 60 \times 12 \times \cos 60° = 60 \times 12 \times 0{,}5 = \SI{360}{\joule}$$
>
> **Contrôle.** L'angle est inférieur à $90°$, donc le cosinus est positif, donc le travail
> est **moteur** : la force aide au déplacement. Et l'unité est bien le joule.

---

## Les pièges de ce domaine

- Oublier le facteur $\frac{1}{2}$ dans l'énergie cinétique.
- Élever la masse au carré au lieu de la vitesse.
- Appliquer la conservation de l'énergie mécanique en présence de frottement.
- Dire que l'énergie dissipée est perdue : elle est passée en énergie interne.

## Ton entraînement

**Exercice 1.** Une caisse de $\SI{15}{\kilogram}$ est tirée horizontalement sur
$\SI{8}{\metre}$ par une force horizontale de $\SI{40}{\newton}$. On prendra
$g =  \SI{9.8} {\newton\per\kilogram}$.

Travail de la force de traction : ..............................  Unité : ..........

Travail du poids : ..............................  Justification : ..........................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 2.** Un objet de $\SI{2.0}{\kilogram}$ se déplace à $\SI{6.0}{\metre\per\second}$.
Calculer son énergie cinétique. Puis recalculer si sa vitesse double.

$E_c$ à $\SI{6.0}{\metre\per\second}$ : ..............................

$E_c$ à $\SI{12.0}{\metre\per\second}$ : ..............................

Par combien l'énergie a-t-elle été multipliée ? ..................  Pourquoi ? ................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 3.** Le même objet de $\SI{2.0}{\kilogram}$ est posé, immobile, sur une étagère à
$\SI{1.5}{\metre}$ du sol. Calculer son énergie cinétique et son énergie potentielle de
pesanteur.

$E_c =$ ..............................  $E_{pp} =$ ..............................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 4.** Une bille tombe en chute libre d'une hauteur de $\SI{5.0}{\metre}$. Que vaut
le travail de son poids sur cette descente ? Ce travail est-il moteur ou résistant ?

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

## Tes exercices, ceux qui viennent de ton positionnement

**1. Optique — Situer l'image d'un objet à l'infini.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Une lentille convergente a une distance focale de $\SI{5}{\centi\metre}$. Où se forme l'image du Soleil ? Et celle d'un objet placé à $\SI{15}{\centi\metre}$ devant la lentille ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Suppose que la lentille conserve la position de l'objet, ce qui correspondrait à un système afocal.

> **Méthode.** Les rayons issus d'un objet à l'infini arrivent parallèles entre eux : ils convergent dans le plan focal image. Pour un objet à distance finie, appliquer la relation de conjugaison.

**2. Optique — Utiliser les rayons particuliers d'une lentille.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Citer les trois rayons particuliers d'une lentille convergente et décrire le comportement de chacun à la traversée de la lentille.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Attribue à une lentille le comportement d'un miroir.

> **Méthode.** Trois rayons se tracent sans calcul : par le centre optique, parallèle à l'axe, et par le foyer objet. Deux suffisent pour construire une image.

### Si tu bloques — ouvre les indices dans l'ordre

N'ouvre l'indice suivant que si le précédent n'a pas suffi, et note lequel t'a débloqué : « je ne savais pas » et « il me manquait un déclencheur » ne se traitent pas de la même façon.

- **Indice 1 — orientation.** Relis l'énoncé et demande-toi de quel domaine il relève. Écris le nom de la propriété avant toute chose.
- **Indice 2 — méthode.** Reprends la méthode de la page précédente, étape par étape, et applique la première à ton énoncé.
- **Indice 3 — première étape.** Pose l'écriture de départ, sans la résoudre. Si tu y arrives, la suite t'appartient.

Indice qui m'a débloqué : $\square$1 $\square$2 $\square$3 $\square$aucun, j'ai trouvé seul

## Transfert — à toi de choisir la méthode

Rien dans cet énoncé ne dit quelle notion employer. C'est la question.

Un ascenseur de $\SI{800}{\kilogram}$ monte de $\SI{15}{\metre}$ à vitesse
constante en $\SI{20}{\second}$. a) Quelle énergie le moteur a-t-il fournie, au minimum ?
b) Quelle puissance cela représente-t-il ? c) La puissance réelle du moteur est de
$\SI{8}{\kilo\watt}$. Que devient la différence ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

## Pour aller vers la Terminale

Ce qui suit n'est pas attendu de toi aujourd'hui. C'est là pour que tu voies à quoi sert ce que tu viens de travailler.

Une voiture freine et s'arrête. Son énergie cinétique valait $\SI{200}{\kilo\joule}$. Où
est passée cette énergie ?

....................................................................................................

Le bilan de Première ne suit que $E_c$ et $E_{pp}$. Quand il y a frottement, l'énergie
mécanique diminue : elle n'a pas disparu, elle est passée sous une autre forme,
l'**énergie interne**.

Le **premier principe** de la thermodynamique complète le bilan :

$$\Delta U = W + Q$$

où $Q$ est le transfert thermique. Rien n'est perdu ; tout est compté. C'est sur cette
relation que la Terminale construit le flux thermique, la résistance thermique et la loi
de refroidissement de Newton.

---

## Prendre du recul

- Comment contrôler qu'une énergie calculée a le bon ordre de grandeur ?

....................................................................................................

- Pourquoi faut-il annoncer l'origine des altitudes avant de calculer une énergie potentielle ?

....................................................................................................

## Avant de partir

Trois questions courtes. Elles disent au professeur ce qui est acquis, ce qui est à confirmer, et ce qui doit être repris à la séance suivante.

**1.** Énergie cinétique d'un objet de $\SI{4.0}{\kilogram}$ à $\SI{5.0}{\metre\per\second}$.

....................................................................................................

**2.** Quelle est l'unité de l'énergie ?

....................................................................................................

**3.** En présence de frottement, l'énergie mécanique se conserve-t-elle ?

....................................................................................................

### D'ici la prochaine séance — quinze minutes, pas davantage

Relis la fiche de cours en mécanique et note ce qui reste flou : tes questions feront gagner du temps à tout le monde.

Fait : $\square$oui $\square$non    Ce que j'ai noté : ..................................................

**Ma certitude sur cette séance :** $\square$1 $\square$2 $\square$3 $\square$4  
**L'aide maximale que j'ai utilisée :** $\square$A $\square$B $\square$C $\square$D $\square$E $\square$aucune

---

<div class="page-break"></div>

# Séance 4 — Ondes et optique : période, célérité, longueur d'onde, lentilles minces

**Le thème du groupe aujourd'hui :** Ondes et optique : période, célérité, longueur d'onde, lentilles minces.  
**Ton point personnel pendant le temps différencié :** Transformations chimiques.  
**Ta piste :** Installer. Écris la propriété ou la relation que tu utilises **avant** de calculer.

## Aujourd'hui, tu vas…

Construire les premiers automatismes sur les transformations chimiques.

L'entraînement collectif porte sur ondes et optique ; ton exercice personnel, plus bas, porte sur transformations chimiques. Les deux sont à traiter : le premier avec le groupe, le second pendant le temps différencié.

> **Remarque.** Ce que ton positionnement a montré sur ce point : Transformations chimiques (INSTALLER). Sur ce domaine, tu as réussi 25 % des questions du positionnement. C'est de là que part ta séance.

> **Rappel.** Un point sur lequel tu peux t'appuyer aujourd'hui : Électricité — réponses justes, données avec assurance : c'est un vrai point d'appui pour la suite.

## Pour commencer — dix minutes, de tête

Ces questions ne sont pas notées. Elles servent à réveiller ce dont la séance a besoin. Si l'une te bloque, signale-la : c'est une information utile.

1. Relation entre longueur d'onde, célérité et fréquence.  ....................
2. Relation entre période et fréquence.  ....................
3. Unité de la fréquence.  ....................
4. $\SI{340}{\metre\per\second}$ et $\SI{170}{\hertz}$ : quelle longueur d'onde ?  ....................
5. Quand une onde change de milieu, qu'est-ce qui ne change pas ?  ....................
6. Où se forme l'image d'un objet situé à l'infini ?  ....................
7. Un rayon passant par le centre optique : que devient-il ?  ....................

> **Rappel.** Tu as travaillé **Mécanique** en séance 2. Avant de commencer, écris en une phrase la règle que tu en as retenue — sans regarder tes notes. Si elle ne vient pas, c'est le moment de le dire.

## L'essentiel à retenir

> **La relation des ondes.**
> $$\lambda = v \times T = \frac{v}{f}$$
>
> | Grandeur | Imposée par | Change en changeant de milieu ? |
> |---|---|---|
> | Fréquence $f$ | la **source** | **non** |
> | Période $T = 1/f$ | la **source** | **non** |
> | Célérité $v$ | le **milieu** | **oui** |
> | Longueur d'onde $\lambda$ | les deux | **oui** |
>
> **Contrôle par les unités.** $\si{\metre} = \dfrac{ \si{\metre\per\second} }{ \si{\per\second} }$
> est cohérent. Le produit $v \times f$ ne l'est pas : il ne donne pas une longueur.
>
> **Contrôle qualitatif.** Plus la fréquence est grande, plus la longueur d'onde est
> **petite**. Un son aigu a une longueur d'onde plus courte qu'un son grave.
>
> **Les trois rayons particuliers d'une lentille mince convergente.**
>
> 1. Un rayon **parallèle à l'axe optique** émerge en passant par le foyer image $F'$.
> 2. Un rayon **passant par le foyer objet** $F$ émerge parallèle à l'axe optique.
> 3. Un rayon **passant par le centre optique** $O$ **n'est pas dévié**.
>
> **Objet à l'infini.** Les rayons arrivent parallèles entre eux. Ils convergent tous dans
> le **plan focal image**. Pas au centre optique : le centre optique est un point de la
> lentille, pas un lieu de convergence.

---

## La méthode, dans l'ordre

1. J'écris la relation avant de remplacer par les valeurs.
2. Je convertis toutes les grandeurs dans les unités du système international.
3. Pour une lentille, je trace les deux rayons particuliers avant d'en tracer un troisième.
4. Je repère l'image à l'intersection des rayons émergents, ou de leurs prolongements.
5. Je contrôle l'ordre de grandeur et l'unité.

## Un exemple mené jusqu'au bout

> **Exemple, en ondes.** Un son de $\SI{200}{\hertz}$ se propage dans l'air à
> $\SI{340}{\metre\per\second}$.
> $$\lambda = \frac{340}{200} = \SI{1.7}{\metre}$$
> **Contrôle :** un son grave (fréquence faible) a une longueur d'onde **grande**. Ici
> $\SI{1.7}{\metre}$ pour $\SI{200}{\hertz}$, contre $\SI{0.34}{\metre}$ pour
> $\SI{1000}{\hertz}$. C'est cohérent.
>
> **Exemple, en optique.** Objet $AB$ placé avant $F$. Rayon 1 : de $B$, horizontal, puis
> par $F'$. Rayon 3 : de $B$ vers $O$, tout droit. Les deux se croisent après la
> lentille : c'est $B'$.

---

## Les pièges de ce domaine

- Croire que la fréquence change quand l'onde change de milieu : c'est la longueur d'onde qui change.
- Confondre période et fréquence, qui sont inverses l'une de l'autre.
- Tracer un rayon particulier faux, et construire l'image dessus sans le voir.
- Croire qu'en masquant la moitié d'une lentille on perd la moitié de l'image.

## Ton entraînement

**Exercice 1.** Un son de fréquence $\SI{440}{\hertz}$ se propage dans l'air, où la célérité
vaut $\SI{340}{\metre\per\second}$. Calculer sa longueur d'onde.

Relation utilisée : ......................................................................

Calcul : ...........................................................................................

Résultat : ..............................  Unité : ..........

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 2.** Le même son passe dans l'eau, où la célérité vaut
$\SI{1500}{\metre\per\second}$. Que valent sa fréquence et sa longueur d'onde dans l'eau ?

Fréquence : ..............................  Longueur d'onde : ..............................

Justification de la fréquence : ...........................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 3.** Sur le schéma ci-dessous, trace les trois rayons particuliers issus du
point $B$, puis place l'image $B'$.

```{=latex}
\begin{center}
\begin{tikzpicture}[x=10mm,y=10mm]
  \draw[TextGray,thin,->] (-5.6,0) -- (5.8,0) node[right,font=\small] {axe optique};
  \draw[Navy,very thick] (0,-2.1) -- (0,2.1);
  \draw[Navy,thick] (-0.22,1.88) -- (0,2.1) -- (0.22,1.88);
  \draw[Navy,thick] (-0.22,-1.88) -- (0,-2.1) -- (0.22,-1.88);
  \foreach \x/\lab/\pos in {-2.5/F/below,2.5/{F'}/below,0/O/{below left}}
     {\fill (\x,0) circle (1.7pt); \node[\pos=1mm,font=\small] at (\x,0) {$\lab$};}
  \draw[->,Red,very thick] (-4.2,0) -- (-4.2,1.5) node[above,font=\small] {$B$};
  \node[Red,font=\small,below] at (-4.2,0) {$A$};
\end{tikzpicture}
\end{center}
```

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

**Exercice 4.** Un objet est situé à l'infini. Où se forme son image ? Cocher la bonne
réponse et justifier.

$\square$ au centre optique  $\square$ dans le plan focal image  $\square$ à l'infini

Justification : ...........................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4   Aide utilisée : $\square$A $\square$B $\square$C $\square$D $\square$E

## Tes exercices, ceux qui viennent de ton positionnement

**1. Transformations chimiques — Identifier le réactif limitant.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Pour la transformation d'équation $\ce{A + 3 B -> C}$, on introduit $\SI{0.5}{\mole}$ de A et $\SI{1.2}{\mole}$ de B. Quel est le réactif limitant ? Justifier.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Croit qu'un raisonnement sur les quantités de matière exige de repasser par les masses.

> **Méthode.** Ne jamais comparer les quantités brutes : diviser d'abord chaque quantité par le coefficient stœchiométrique du réactif correspondant.

**2. Transformations chimiques — Lire un couple oxydant-réducteur.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Écrire la demi-équation électronique du couple $\ce{Fe^3+}$ / $\ce{Fe^2+}$ et préciser laquelle des deux espèces est l'oxydant.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Nie la structure même d'un couple, qui associe toujours un oxydant et un réducteur.

> **Méthode.** Un couple s'écrit toujours oxydant / réducteur, dans cet ordre. La demi-équation s'écrit avec les électrons du côté de l'oxydant.

**3. Transformations chimiques — Appliquer la définition de Brønsted.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Écrire l'équation de la réaction entre l'acide éthanoïque $\ce{CH3COOH}$ et l'ion hydroxyde $\ce{HO^-}$, puis identifier les deux couples acide/base mis en jeu.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Reprend la définition d'Arrhenius d'une base, et non celle de Brønsted d'un acide.

> **Méthode.** Au sens de Brønsted, un acide **donne** un proton $\ce{H^+}$, une base le **capte**. Les deux formes d'une même espèce constituent un couple acide/base.

**4. Transformations chimiques — Relier pH et concentration en ions oxonium.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réponse fausse au positionnement.

Une solution a un pH égal à 2. Quelle est sa concentration en ions $\ce{H3O^+}$ ? Que devient son pH si on la dilue cent fois ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Piège.** Inverse l'échelle du pH, où les valeurs faibles correspondent à l'acidité.

> **Méthode.** pH = $- \log$[$\ce{H3O^+}$], donc [$\ce{H3O^+}$] $= 10^{ - \mathrm{pH}}$. Une unité de pH correspond à un facteur dix sur la concentration.

### Si tu bloques — ouvre les indices dans l'ordre

N'ouvre l'indice suivant que si le précédent n'a pas suffi, et note lequel t'a débloqué : « je ne savais pas » et « il me manquait un déclencheur » ne se traitent pas de la même façon.

- **Indice 1 — orientation.** Relis l'énoncé et demande-toi de quel domaine il relève. Écris le nom de la propriété avant toute chose.
- **Indice 2 — méthode.** Reprends la méthode de la page précédente, étape par étape, et applique la première à ton énoncé.
- **Indice 3 — première étape.** Pose l'écriture de départ, sans la résoudre. Si tu y arrives, la suite t'appartient.

Indice qui m'a débloqué : $\square$1 $\square$2 $\square$3 $\square$aucun, j'ai trouvé seul

## Transfert — à toi de choisir la méthode

Rien dans cet énoncé ne dit quelle notion employer. C'est la question.

Une chauve-souris émet un cri de $\SI{40}{\kilo\hertz}$ dans l'air
($v = \SI{340}{\metre\per\second}$) et perçoit l'écho $\SI{12}{\milli\second}$ plus tard.
a) Quelle est la longueur d'onde de son cri ? b) À quelle distance se trouve l'obstacle ?
Attention au trajet parcouru. c) Pourquoi une fréquence élevée est-elle un avantage ici ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

## Pour aller vers la Terminale

Ce qui suit n'est pas attendu de toi aujourd'hui. C'est là pour que tu voies à quoi sert ce que tu viens de travailler.

**Diffraction et interférences.** Les deux se quantifient à partir de $\lambda$. L'écart
angulaire de diffraction vaut $\theta = \lambda / a$, où $a$ est la largeur de la fente :
la longueur d'onde d'aujourd'hui, dans une relation de demain.

**Effet Doppler.** Quand une source s'**approche**, la fréquence perçue augmente. Ce n'est
pas une contradiction : ici ce n'est pas le milieu qui change, c'est le mouvement relatif
de la source et de l'observateur.

**Lunette astronomique.** Deux lentilles convergentes en série. Un objet à l'infini donne
une image dans le plan focal de la première, qui sert d'objet à la seconde. Les trois
rayons d'aujourd'hui suffisent à construire toute la lunette.

**Et une nouveauté :** la lumière décrite comme un flux de **photons**, avec des
transitions d'énergie. Là, le tracé géométrique cède la place à un bilan énergétique.

---

## Prendre du recul

- Comment vérifier qu'une longueur d'onde calculée est plausible ?

....................................................................................................

- Quelle grandeur ne change pas quand une onde change de milieu ?

....................................................................................................

## Avant de partir

Trois questions courtes. Elles disent au professeur ce qui est acquis, ce qui est à confirmer, et ce qui doit être repris à la séance suivante.

**1.** Relation entre longueur d'onde, célérité et fréquence.

....................................................................................................

**2.** $\SI{340}{\metre\per\second}$ et $\SI{680}{\hertz}$ : quelle longueur d'onde ?

....................................................................................................

**3.** Où se forme l'image d'un objet situé à l'infini ?

....................................................................................................

### D'ici la prochaine séance — quinze minutes, pas davantage

Et pendant tout le stage, garde le réflexe du bilan : avant de valider une réponse, demande-toi « j'en suis sûr, ou je crois l'être ? ».

Fait : $\square$oui $\square$non    Ce que j'ai noté : ..................................................

**Ma certitude sur cette séance :** $\square$1 $\square$2 $\square$3 $\square$4  
**L'aide maximale que j'ai utilisée :** $\square$A $\square$B $\square$C $\square$D $\square$E $\square$aucune

---

<div class="page-break"></div>

# Séance 5 — Électricité, chimie organique, mesure et incertitudes, évaluation

**Le thème du groupe aujourd'hui :** Électricité, chimie organique, mesure et incertitudes, évaluation.  
**Ton point personnel pendant le temps différencié :** Chimie organique.  
**Ta piste :** Consolider. Justifie par écrit, et sans carte d'aide. Tu sais faire ; il reste à le faire sans hésiter.

## Aujourd'hui, tu vas…

Stabiliser l'acquis encore fragile en chimie organique.

L'entraînement collectif porte sur électricité, chimie organique, mesure et incertitudes, évaluation ; ton exercice personnel, plus bas, porte sur chimie organique. Les deux sont à traiter : le premier avec le groupe, le second pendant le temps différencié.

> **Remarque.** Ce que ton positionnement a montré sur ce point : Chimie organique (CONSOLIDER). Sur ce domaine, tu as réussi 100 % des questions du positionnement. C'est de là que part ta séance.

> **Rappel.** Un point sur lequel tu peux t'appuyer aujourd'hui : Électricité — réponses justes, données avec assurance : c'est un vrai point d'appui pour la suite.

## Pour commencer — dix minutes, de tête

Ces questions ne sont pas notées. Elles servent à réveiller ce dont la séance a besoin. Si l'une te bloque, signale-la : c'est une information utile.

1. Loi d'Ohm.  ....................
2. Formule de la puissance dissipée par un conducteur ohmique, à partir de R et I.  ....................
3. Relation entre énergie, puissance et durée.  ....................
4. Unité de la puissance. Unité de l'énergie.  ....................
5. $\SI{20}{\ohm}$ traversé par $\SI{0.50}{\ampere}$ : quelle tension ?  ....................
6. Combien de chiffres significatifs dans $\SI{0.0250}{\metre}$ ?  ....................
7. Le courant est-il le même avant et après une résistance ?  ....................

> **Rappel.** Tu as travaillé **Optique** en séance 3. Avant de commencer, écris en une phrase la règle que tu en as retenue — sans regarder tes notes. Si elle ne vient pas, c'est le moment de le dire.

## La méthode, dans l'ordre

1. J'écris la relation littérale avant de remplacer par les valeurs.
2. Je convertis les durées en secondes avant tout calcul d'énergie.
3. Je calcule, puis j'arrondis en dernier — jamais en cours de calcul.
4. Je fixe le nombre de chiffres significatifs d'après la donnée la moins précise.
5. J'écris le résultat avec son unité, et je contrôle son ordre de grandeur.

> **Méthode.** Aucun exemple résolu ne t'est donné ici : sur ce domaine, tu réussis. Ce qui est attendu de toi aujourd'hui, c'est la **rédaction** — la propriété écrite avant le calcul, la conclusion qui répond à la question posée.

## Les pièges de ce domaine

- Croire que l'intensité diminue après la traversée d'une résistance.
- Laisser une durée en minutes dans un calcul d'énergie en joules.
- Recopier tous les chiffres de la calculatrice comme s'ils étaient significatifs.
- Arrondir à chaque étape intermédiaire, ce qui déplace le résultat final.

## Ton entraînement

### Partie 1 — Électricité

> **Puissance électrique.**
> $$P = U \times I$$
> $P$ en watts ($\si{\watt}$), $U$ en volts ($\si{\volt}$), $I$ en ampères ($\si{\ampere}$).
>
> Pour un conducteur ohmique, $U = R \times I$, donc :
> $$P = R \, I^{2}$$
>
> **Effet Joule.** L'énergie électrique reçue par un conducteur est dissipée sous forme
> **thermique** : le conducteur chauffe.
>
> Ce n'est **pas** une perte de courant. Le courant qui sort d'une résistance est le même
> que celui qui y entre. C'est l'**énergie** qui change de forme, pas le courant qui
> disparaît.
>
> **Puissance et énergie.** La puissance est une énergie **par seconde** :
> $$E = P \times \Delta t \qquad \text{en joules, avec } \Delta t \text{ en secondes}$$

**Exercice 1.** Un conducteur ohmique de résistance $\SI{50}{\ohm}$ est traversé par un
courant de $\SI{0.20}{\ampere}$. Calculer la tension à ses bornes et la puissance dissipée.

Relations utilisées : ....................................................................

$U =$ ..............................  $P =$ ..............................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4

**Exercice 2.** Le même conducteur fonctionne pendant $\SI{5.0}{\minute}$. Quelle énergie
a-t-il dissipée ? Sous quelle forme ?

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4

**Exercice 3.** Vrai ou faux, et pourquoi : « après avoir traversé une résistance, le
courant est plus faible, puisque de l'énergie a été perdue. »

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

Certitude : $\square$1 $\square$2 $\square$3 $\square$4

---

### Partie 2 — Chimie organique (entretien)

Ce domaine est celui que tu réussis. On l'entretient parce que la Terminale construit
dessus sans y revenir.

> | Groupe | Exemple | Famille |
> |---|---|---|
> | $\ce{-OH}$ | $\ce{CH3-CH2-OH}$ | alcool |
> | $\ce{-CHO}$ | $\ce{CH3-CHO}$ | aldéhyde |
> | $\ce{-CO-}$ | $\ce{CH3-CO-CH3}$ | cétone |
> | $\ce{-COOH}$ | $\ce{CH3-COOH}$ | acide carboxylique |
> | $\ce{-COO-}$ | $\ce{CH3-COO-CH3}$ | ester |

**Exercice 4.** Pour chacune de ces formules, entoure le groupe caractéristique et nomme
la famille. Trente secondes par formule.

| Formule | Groupe entouré | Famille |
|---|---|---|
| $\ce{CH3-CH2-CH2-OH}$ | | |
| $\ce{CH3-CH2-COOH}$ | | |
| $\ce{CH3-CO-CH2-CH3}$ | | |
| $\ce{H-CHO}$ | | |
| $\ce{CH3-CH2-COO-CH3}$ | | |

Certitude : $\square$1 $\square$2 $\square$3 $\square$4

**Ce que la Terminale en fera.** Elle passe de la **reconnaissance** à la **réactivité** :
polarisation des liaisons, sites donneurs et accepteurs de doublets, mécanismes
réactionnels étape par étape, puis stratégie de synthèse. Un groupe non identifié rend le
mécanisme illisible.

---

### Partie 3 — Rendre un résultat de mesure

> **Trois règles.**
>
> 1. **Un résultat sans unité n'est pas un résultat.**
> 2. Le nombre de **chiffres significatifs** ne dépasse jamais celui de la donnée la moins
>    précise.
> 3. L'**ordre de grandeur** se contrôle avant de conclure.

**Exercice 5.** Une distance de $\SI{1.2}{\metre}$ est parcourue en $\SI{0.35}{\second}$. La
calculatrice affiche $3{,}428571429$. Quel résultat rends-tu ?

....................................................................................................

Pourquoi pas tous les chiffres ? ..........................................................

**Exercice 6.** Trois résultats sont proposés. Un seul est acceptable. Lequel, et pourquoi
les deux autres ne le sont-ils pas ?

- (a) La longueur d'onde vaut $0{,}77$.
- (b) La longueur d'onde vaut $\SI{0.7727272}{\metre}$.
- (c) La longueur d'onde vaut $\SI{0.77}{\metre}$.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

**Exercice 7.** Contrôle d'ordre de grandeur. Pour chaque résultat, dis s'il est plausible.

| Résultat | Plausible ? | Pourquoi |
|---|---|---|
| Vitesse d'un piéton : $\SI{50}{\metre\per\second}$ | | |
| Longueur d'onde d'un son audible : $\SI{300}{\metre}$ | | |
| Énergie cinétique d'une voiture sur autoroute : $\SI{500}{\kilo\joule}$ | | |
| Énergie potentielle gagnée en montant un étage : $\SI{2}{\kilo\joule}$ | | |

....................................................................................................

....................................................................................................

....................................................................................................

---

## Tes exercices, ceux qui viennent de ton positionnement

**1. Chimie organique — Associer un groupe caractéristique à sa famille.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réussi avec une certitude de 2/4.

Donner la famille et le groupe caractéristique de $\ce{CH3} -$CO$- \ce{CH3}$, puis ceux de $\ce{CH3} -$CHO.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Méthode.** Repérer d'abord l'atome porteur, puis le voisinage : $-$COOH réunit un carbonyle et un hydroxyle sur le même carbone, ce qui le distingue de l'alcool et de la cétone.

**2. Chimie organique — Identifier une famille à partir d'une formule semi-développée.** Cet exercice est là parce que, au positionnement, la question correspondante a donné une réussi avec une certitude de 2/4.

Nommer $\ce{CH3} - \ce{CH2} - \ce{CH2} -$OH et donner sa famille. À quelle famille appartient $\ce{CH3} -$CO$- O - \ce{CH3}$ ?

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

> **Méthode.** Un $-$OH porté par un carbone tétragonal caractérise un alcool ; le même $-$OH voisin d'un carbonyle donnerait un acide carboxylique.

### Si tu bloques — ouvre les indices dans l'ordre

N'ouvre l'indice suivant que si le précédent n'a pas suffi, et note lequel t'a débloqué : « je ne savais pas » et « il me manquait un déclencheur » ne se traitent pas de la même façon.

- **Indice 1 — orientation.** Relis l'énoncé et demande-toi de quel domaine il relève. Écris le nom de la propriété avant toute chose.
- **Indice 2 — méthode.** Reprends la méthode de la page précédente, étape par étape, et applique la première à ton énoncé.
- **Indice 3 — première étape.** Pose l'écriture de départ, sans la résoudre. Si tu y arrives, la suite t'appartient.

Indice qui m'a débloqué : $\square$1 $\square$2 $\square$3 $\square$aucun, j'ai trouvé seul

## Transfert — à toi de choisir la méthode

Rien dans cet énoncé ne dit quelle notion employer. C'est la question.

Ici, on attend une rédaction complète : la propriété citée, le calcul mené, la conclusion écrite en français.

Une plaque chauffante porte l'indication « $\SI{230}{\volt}$ —
$\SI{2000}{\watt}$ ». a) Quelle intensité la traverse en fonctionnement normal ? b) Quelle est
sa résistance ? c) Elle chauffe $\SI{1.5}{\litre}$ d'eau pendant $\SI{6.0}{\minute}$. Quelle
énergie a-t-elle consommée ? d) Écris chaque résultat avec un nombre de chiffres significatifs
justifié.

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

....................................................................................................

## Prendre du recul

- Comment contrôler qu'une puissance calculée est plausible pour un appareil domestique ?

....................................................................................................

- Combien de chiffres significatifs peut-on garder quand la donnée la moins précise en a deux ?

....................................................................................................

## Avant de partir

Trois questions courtes. Elles disent au professeur ce qui est acquis, ce qui est à confirmer, et ce qui doit être repris à la séance suivante.

**1.** Loi d'Ohm.

....................................................................................................

**2.** $\SI{100}{\ohm}$ traversé par $\SI{0.20}{\ampere}$ : quelle puissance dissipée ?

....................................................................................................

**3.** Le courant est-il le même avant et après une résistance ?

....................................................................................................

**Ma certitude sur cette séance :** $\square$1 $\square$2 $\square$3 $\square$4  
**L'aide maximale que j'ai utilisée :** $\square$A $\square$B $\square$C $\square$D $\square$E $\square$aucune

---

<div class="page-break"></div>

## Bilan personnel — à remplir à la fin du stage

Rien n'est prérempli ici : ce sont des constats, et ils n'existent qu'après les cinq séances.

| Séance | 1 | 2 | 3 | 4 | 5 |
|---|:---:|:---:|:---:|:---:|:---:|
| Aide maximale utilisée | | | | | |
| Certitude déclarée | | | | | |

**Ce que je consolide vraiment, et que je saurais refaire seul**

....................................................................................................

....................................................................................................

**Le point que je dois encore surveiller à la rentrée**

....................................................................................................

....................................................................................................

**Une méthode que je retiens, écrite avec mes mots**

....................................................................................................

....................................................................................................

**La première notion de Terminale que je me sens prêt à aborder**

....................................................................................................

....................................................................................................
