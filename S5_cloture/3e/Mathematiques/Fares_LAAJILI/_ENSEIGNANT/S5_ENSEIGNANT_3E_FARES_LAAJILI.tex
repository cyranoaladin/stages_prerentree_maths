...
\usepackage{nexusS5}
\setnexusheader{Dossier enseignant — confidentiel}{S5 — Fares LAAJILI — usage enseignant}
\begin{document}
\nexuscover{SÉANCE 5 --- DOSSIER ENSEIGNANT}{Profil, déroulé, corrigés et barème}{Fares LAAJILI}{Entrée en Troisième}{Mathématiques --- Consolidation fractions et trigonométrie}
...
\begin{methodbox}
\textbf{Diagnostic initial.} Diagnostic initial (18 items). Six domaines solides ; multiplication de fractions, Pythagore et cosinus à rectifier.
\end{methodbox}
\subsectiontitle{Points d'appui documentés}
\begin{itemize}\item Relatifs\item Puissances\item Calcul littéral\item Équations\item Proportionnalité\item Statistiques\end{itemize}
\subsectiontitle{Difficultés documentées}
\begin{itemize}\item Produit de fractions\item Pythagore sur les carrés\item Cosinus versus sinus\end{itemize}
...
M3E\_LIT\_02 & Réduction d'une expression avec constantes signées & S2 & acquis & OK \\
...
M3E\_STAT\_01 & Moyenne simple et moyenne pondérée & S5 & acquis & OK \\
...
M3E\_TRIG\_01 & $\bullet$ Choix et emploi d'un rapport trigonométrique (cosinus, sinus) & S4 & fragile & P1 \\
...
\begin{warningbox}\textbf{Effet de récence.} Les compétences suivantes sont retravaillées pendant cette séance, puis évaluées moins d'une heure plus tard : \code{M3E_STAT_01}, \code{M3E_FRAC_02}, \code{M3E_TRIG_01}. Une réussite y mesure une \emph{performance immédiate}, pas une consolidation durable. Le plan de rentrée prévoit pour elles un contrôle différé en semaine 1 ou 2 ; aucun bilan ne doit écrire « acquis durablement » sur cette seule preuve.\end{warningbox}
\sectiontitle{3. Pourquoi ces activités, pour cet élève}
\begin{goalbox}\textbf{Objectif de la séance :} Stabiliser le produit et le quotient de fractions, puis choisir le rapport trigonométrique à partir du nom des côtés et non d'une habitude.\end{goalbox}
...
\par\vspace{2mm}
\itemhead{A5 --- \code{M3E_FRAC_02} --- item individualisé}{1}{1.5}
\begin{graybox}\small\textbf{Réponse attendue :} $\dfrac{4}{5}$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $\frac{6}{25}\times\frac{10}{3}=\frac{60}{75}$\item simplification par $15$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_FRAC\_02 & automatisme & $\frac45$ ou une écriture équivalente exacte \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & division effectuée terme à terme \\
METHODE & inverse pris sur la première fraction \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} quotients de fractions employés dans les proportions et les probabilités
\par\vspace{2mm}
\itemhead{A6 --- \code{M3E_TRIG_01} --- item individualisé}{1}{1.5}
\begin{graybox}\small\textbf{Réponse attendue :} Le sinus : $\sin\widehat{C}=\dfrac{\text{opposé}}{\text{hypoténuse}}$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item identifier les deux côtés fournis\item choisir le rapport qui les met en jeu\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_TRIG\_01 & procedure & rapport nommé et défini correctement \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & cosinus choisi par habitude \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} choix du rapport, compétence évaluée tout au long de la Troisième
\par\vspace{2mm}
...
\begin{graybox}\small\textbf{Réponse attendue :} $BC = 29$ cm ; l'élève a doublé les longueurs au lieu de les élever au carré : $400 + 441 = 841$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $BC^2 = 400 + 441 = 841$\item $BC = 29$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_GEO\_01 & automatisme & $BC = 29$ cm exact \\
c2 & 1 & M3E\_GEO\_01 & justification & erreur correctement nommée et rectifiée \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & carré confondu avec le double \\
CALCUL & racine carrée de 841 erronée \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} Pythagore réinvesti en géométrie repérée et en trigonométrie
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{C2 --- \code{M3E_TRIG_01} --- item individualisé}{2}{4.5}
\begin{graybox}\small\textbf{Réponse attendue :} $AC$ est opposé à $\widehat{B}$ et $BC$ est l'hypoténuse : c'est le sinus. $\sin\widehat{B}=\frac{10}{26}\approx 0{,}3846$ donc $\widehat{B}\approx 23^\circ$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item nommer les côtés relativement à $\widehat{B}$\item $\sin\widehat{B}=\frac{10}{26}$\item $\widehat{B}\approx 23^\circ$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M3E\_TRIG\_01 & justification & rapport correctement choisi et justifié \\
c2 & 1 & M3E\_TRIG\_01 & automatisme & mesure exacte au degré près \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & cosinus employé alors que le côté est opposé \\
CALCUL & calculatrice en radians \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} calcul d'angle dans une configuration non standard
\par\vspace{2mm}
...
B4 & M3E\_GEO\_01 & 3E\_TI\_Q13 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
C2 & M3E\_TRIG\_01 & 3E\_TI\_Q16 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
\end{tabularx}
...
\begin{itemize}\item Indicateurs du dossier : cinq produits de fractions, trois problèmes de Pythagore, quatre rapports trigonométriques identifiés.\item Vérifier que les côtés sont nommés à voix haute avant l'écriture du rapport.\item Contrôler que les carrés sont bien élevés au carré et non doublés dans l'application de Pythagore.\end{itemize}
