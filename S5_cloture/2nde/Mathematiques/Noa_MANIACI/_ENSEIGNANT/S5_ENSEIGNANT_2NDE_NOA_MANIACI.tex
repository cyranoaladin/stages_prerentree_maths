...
\usepackage{nexusS5}
\setnexusheader{Dossier enseignant — confidentiel}{S5 — Noa MANIACI — usage enseignant}
\begin{document}
\nexuscover{SÉANCE 5 --- DOSSIER ENSEIGNANT}{Profil, déroulé, corrigés et barème}{Noa MANIACI}{Entrée en Seconde générale et technologique}{Mathématiques --- Installer les équations et les évolutions}
...
\begin{methodbox}
\textbf{Diagnostic initial.} Diagnostic initial (18 items). Sept domaines comportent des certitudes erronées ; équations à installer.
\end{methodbox}
\subsectiontitle{Points d'appui documentés}
\begin{itemize}\item Équation linéaire ponctuellement réussie\item Réciproque 6-8-10 reconnue\item Facteur d'aire 9 trouvé\item Écriture scientifique ponctuellement correcte\end{itemize}
\subsectiontitle{Difficultés documentées}
\begin{itemize}\item Priorités et signes\item Fractions\item Exposants négatifs\item Double distributivité\item Produit nul\item Évolutions successives\item Pythagore/Thalès\item Trigonométrie\end{itemize}
...
M2DE\_ALG\_01 & Double distributivité et identités remarquables & S2 & fragile & P1 \\
M2DE\_CALC\_01 & Priorités opératoires, signes et puissance d'un relatif & S1 & fragile & P1 \\
M2DE\_EQ\_02 & $\bullet$ Équation produit nul & S2 & en voie & P1 \\
...
M2DE\_POURC\_01 & $\bullet$ Pourcentages, coefficient multiplicateur, évolutions successives & S3 & fragile & P1 \\
M2DE\_PUIS\_01 & Puissances de 10, exposants négatifs, écriture scientifique & S1 & fragile & P1 \\
M2DE\_EQ\_01 & Équation du premier degré, inconnue dans les deux membres & S2 & en voie & P2 \\
M2DE\_TRIG\_01 & Rapports trigonométriques dans le triangle rectangle & S4 & fragile & P2 \\
M2DE\_GEO\_02 & Théorème de Thalès et agrandissement-réduction & S4 & en voie & P3 \\
\end{tabularx}
...
\begin{warningbox}\textbf{Effet de récence.} Les compétences suivantes sont retravaillées pendant cette séance, puis évaluées moins d'une heure plus tard : \code{M2DE_EQ_02}, \code{M2DE_POURC_01}. Une réussite y mesure une \emph{performance immédiate}, pas une consolidation durable. Le plan de rentrée prévoit pour elles un contrôle différé en semaine 1 ou 2 ; aucun bilan ne doit écrire « acquis durablement » sur cette seule preuve.\end{warningbox}
\sectiontitle{3. Pourquoi ces activités, pour cet élève}
\begin{goalbox}\textbf{Objectif de la séance :} Écrire la relation avant le calcul et effectuer un contrôle de vraisemblance — conseil du dossier — en installant d'abord l'équation produit nul puis les évolutions successives.\end{goalbox}
\subsectiontitle{Axe prioritaire (phase 2, 25 min) --- Équation produit nul}
Le dossier place les équations parmi les domaines à installer ; l'équation produit nul est l'outil qui prépare le second degré.
\par\vspace{1mm}\textbf{Compétence visée :} \code{M2DE_EQ_02}.
\subsectiontitle{Second axe (phase 3, 20 min) --- Coefficient multiplicateur et évolutions successives}
Le diagnostic montre que deux évolutions successives sont traitées comme une somme de pourcentages ; c'est une erreur de concept persistante jusqu'en Première.
\par\vspace{1mm}\textbf{Compétence visée :} \code{M2DE_POURC_01}.
...
10--35 min & Équation produit nul & Rappel bref, exemple guidé au tableau, puis exercices en autonomie. Aide donnée seulement après une tentative écrite. & Trois réussites consécutives sans aide sur la compétence visée. \\
35--55 min & Coefficient multiplicateur et évolutions successives & Pas d'exemple guidé : l'élève démarre seul. Intervention uniquement sur demande. & Deux réussites autonomes ; le contrôle est écrit sans qu'on le demande. \\
...
75--120 min & Évaluation finale & Individuel, sans document. Partie A environ 10 min, B environ 20 min, C environ 15 min. & Somme des durées cibles : 39 min, marge de 5 min. \\
\end{tabularx}
...
\end{enumerate}
\subsectiontitle{Phase 2 --- Équation produit nul}
\begin{enumerate}
\item $x=\mathbf{7}$ ou $x=\mathbf{-2}$.
\item $x=\mathbf{-3}$ ou $x=\mathbf{\frac52}$.
\item $x=\mathbf{0}$ ou $x=\mathbf{4}$ : $x$ est lui-même un facteur.
\item $x(x-5)=0$ donc $x=\mathbf{0}$ ou $x=\mathbf{5}$.
\item $(0)\times(-13)=0$ : $-4$ convient. L'autre solution est $x=\mathbf{\frac13}$.
\item Développer donne $3x^2-x-10=0$, équation qu'il ne sait pas encore résoudre : il perd l'accès direct aux deux solutions $-\frac53$ et $2$.
\end{enumerate}
\minititle{Erreurs probables}
\begin{itemize}
\item \textsc{CONCEPT} --- une seule solution retenue
\item \textsc{METHODE} --- forme factorisée développée avant résolution
\item \textsc{JUSTIFICATION} --- propriété du produit nul non énoncée
\end{itemize}
\subsectiontitle{Phase 3 --- Coefficient multiplicateur et évolutions successives}
\begin{enumerate}
\item $340\times 1{,}15 = \mathbf{391}$ TND.
\item $340\times 0{,}85 = \mathbf{289}$ TND.
\item $1{,}08$ et $0{,}92$.
\item Non : $1{,}1\times 1{,}3 = 1{,}43$, soit une hausse globale de $43\,\%$ et non de $40\,\%$.
\end{enumerate}
\minititle{Erreurs probables}
\begin{itemize}
\item \textsc{CONCEPT} --- pourcentages additionnés au lieu de coefficients multipliés
\item \textsc{CONCEPT} --- coefficient d'une baisse de $t\,\%$ pris égal à $t/100$
\item \textsc{CONTROLE} --- sens de l'évolution globale non vérifié
\end{itemize}
...
\begin{keybox}\textbf{Total : 20 points sur 20.} Noyau commun 14 pts (70.0\,\%), items individualisés 6 pts (30.0\,\%). Somme des durées cibles : 39 minutes.\end{keybox}
...
\par\vspace{2mm}
\itemhead{A5 --- \code{M2DE_EQ_02} --- item individualisé}{1}{1.25}
\begin{graybox}\small\textbf{Réponse attendue :} $x = 3$ ou $x = -6$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item produit nul : $4x-12=0$ ou $x+6=0$\item $x=3$ ou $x=-6$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M2DE\_EQ\_02 & procedure & les deux solutions exactes, quelle que soit la méthode employée \\
\bottomrule\end{tabularx}
\begin{successbox}\small\textbf{Méthodes différentes acceptées, sans perte de points :} développer puis résoudre l'équation obtenue : méthode plus longue mais mathématiquement valide ; elle reçoit la totalité des points si les deux solutions sont trouvées\end{successbox}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
CONCEPT & une seule solution donnée \\
CALCUL & erreur sur $4x=12$ ou sur le signe de $-6$ \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} outil direct des équations du second degré
\par\vspace{2mm}
\itemhead{A6 --- \code{M2DE_PUIS_01} --- item individualisé}{1}{1}
\begin{graybox}\small\textbf{Réponse attendue :} $10^{3}$\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item exposants additionnés : $-4+7=3$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M2DE\_PUIS\_01 & automatisme & $10^3$ exact \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
...
CALCUL & signe de l'exposant négatif perdu \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} notation scientifique et ordres de grandeur
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{B4 --- \code{M2DE_POURC_01} --- item individualisé}{2}{3.75}
\begin{graybox}\small\textbf{Réponse attendue :} $180$ TND puis $135$ TND ; coefficient global $0{,}9$, soit une baisse de $10\,\%$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item $150\times 1{,}2 = 180$\item $180\times 0{,}75 = 135$\item $1{,}2\times 0{,}75 = 0{,}9$\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 1 & M2DE\_POURC\_01 & automatisme & les deux prix exacts \\
c2 & 1 & M2DE\_POURC\_01 & automatisme & coefficient global et taux global exacts \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
METHODE & pourcentages additionnés en $-5\,\%$ \\
CONCEPT & baisse appliquée au prix initial \\
CALCUL & produit $1{,}2\times 0{,}75$ mal évalué alors que la méthode est correcte \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} évolutions successives, revues dès le chapitre des pourcentages de Seconde
\par\vspace{2mm}
...
\par\vspace{2mm}
\itemhead{C2 --- \code{M2DE_EQ_02} --- item individualisé}{2}{6.25}
\begin{graybox}\small\textbf{Réponse attendue :} $x(x-7)$ ; $x=0$ ou $x=7$ ; vérification : $0-0=0$ et $49-49=0$.\end{graybox}
\minititle{Étapes significatives}
\begin{enumerate}\item facteur commun $x$\item produit nul\item substitution des deux valeurs\end{enumerate}
\minititle{Barème par critère --- la saisie se fait critère par critère}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\scriptsize}p{9mm} >{\bfseries}p{9mm} >{\ttfamily\scriptsize}p{24mm} >{\scriptsize}p{18mm} X}
\toprule \scriptsize critère & Pts & \scriptsize compétence & \scriptsize preuve & Ce qui est exigé \\ \midrule
c1 & 0.5 & M2DE\_ALG\_01 & procedure & factorisation $x(x-7)$ exacte \\
c2 & 0.5 & M2DE\_EQ\_02 & procedure & les deux solutions exactes \\
c3 & 1 & M2DE\_EQ\_02 & controle & vérification des deux solutions \\
\bottomrule\end{tabularx}
\minititle{Erreurs probables et codes}
\par\noindent\begin{tabularx}{\linewidth}{>{\ttfamily\small}p{26mm} X}
\toprule Code & Description \\ \midrule
METHODE & division des deux membres par $x$, perdant la solution $0$ \\
CONCEPT & $x^2 = 7x$ simplifié en $x = 7$ \\
\bottomrule\end{tabularx}
\par\vspace{1mm}\small\textbf{Rôle pour la rentrée :} factoriser pour résoudre : méthode centrale du second degré
\par\vspace{2mm}
...
A5 & M2DE\_EQ\_02 & 2N\_TI\_Q10 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
A6 & M2DE\_PUIS\_01 & 2N\_TI\_Q05 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
B4 & M2DE\_POURC\_01 & 2N\_TI\_Q12 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
...
C2 & M2DE\_EQ\_02 & 2N\_TI\_Q10 & indicative\_skill\_comparison & confrontation indicative seulement : un énoncé parallèle existe au positionnement initial, mais la réponse de l'élève à cet énoncé n'a pas été conservée \\
\end{tabularx}
...
\begin{itemize}\item Indicateurs du dossier : 80 \% du parcours, aucune répétition des distracteurs initiaux, deux justifications autonomes.\item Vérifier que la propriété du produit nul est énoncée avant la résolution.\item Sur les pourcentages, exiger le coefficient multiplicateur écrit avant tout calcul de prix.\end{itemize}
