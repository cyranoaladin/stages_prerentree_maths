...
from ..domain import points, validation
from ..models import CriterionResponse, ItemObservation
    ...
    for item in assessment.items:
        ...
        for crit in item.criteria:
            ...
            targets = ([(v.virtual_criterion_id, v.description, v.max_score_centi,
                        ...
                        for v in crit.virtual_parts]
                       if crit.curriculum_scope == "mixed"
                       else [(crit.criterion_id, crit.description, crit.max_score_centi,
                              crit.curriculum_scope, crit.analysis_skill_id, False)])
            for scoring_id, desc, max_centi, scope, skill, is_virtual in targets:
                ...
                base["rows"].append({
                    ...
                    "allowed": [{"centi": v, "label": points.format_fr(v)}
...
def correction_screen(student_id: str, request: Request, rapide: int = 0,
    ...
    correction = get_correction(session, assessment, create=not request.state.readonly)
    session.commit()
    ...
    return templates.TemplateResponse(
        ...
        general_fields=corr.GENERAL_OBSERVATION_FIELDS,
...
def validate_correction(student_id: str, request: Request,
    ...
    if problems:
        ...
        request, "partials/validation_problems.html", page_context(
            request, problems=problems, assessment=assessment), status_code=400)
